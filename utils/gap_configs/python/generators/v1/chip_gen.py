  ...
  def __init__(self, tp):
    ...
    if has_cluster:
      for cid in range(0, nb_cluster):
        ...
        cluster_gen_class = tp.get_str('cluster/generator')

        if cluster_gen_class is not None:
          cluster_config = importlib.import_module(cluster_gen_class).Cluster(tp, cid).gen()
        else:
          cluster_config = cluster_gen.Cluster(tp, cid).gen()
