...
void udma::sfu_reset_sync(void *__this, bool value)
{
    udma *_this = (udma *)__this;

    if (_this->sfu_periph)
    {
        _this->sfu_periph->domain_reset(value);
    }
}
...
int udma::build()
{
    ...
    this->sfu_reset_itf.set_sync_meth(&udma::sfu_reset_sync);
    new_slave_port("sfu_reset", &this->sfu_reset_itf);
