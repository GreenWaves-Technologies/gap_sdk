...
Cpi_periph::Cpi_periph(udma *top, int id, int itf_id) : Udma_periph(top, id)
{
  ...
  channel0 = new Cpi_rx_channel(top, this, itf_name + "_rx");
...
vp::io_req_status_e Cpi_periph::custom_req(vp::io_req *req, uint64_t offset)
{
  ...
  switch (offset) {
    case UDMA_CPI_CAM_CFG_GLOB_OFFSET: return handle_global_access(is_write, data);
...
void Cpi_periph::handle_sof()
{
  ...
  if (this->wroteGlob) {
    ...
    this->enabled = ARCHI_REG_FIELD_GET(glob,UDMA_CPI_CAM_CFG_GLOB_EN_BIT, 1);
    this->frameDrop = ARCHI_REG_FIELD_GET(glob,UDMA_CPI_CAM_CFG_GLOB_FRAMEDROP_EN_BIT, 1);
  ...
  if (this->wroteLl) {
    this->wroteLl = false;
    this->frameSliceLlx = ARCHI_REG_FIELD_GET(ll,UDMA_CPI_CAM_CFG_LL_FRAMESLICE_LLX_BIT,UDMA_CPI_CAM_CFG_LL_FRAMESLICE_LLX_WIDTH);
    this->frameSliceLly = ARCHI_REG_FIELD_GET(ll,UDMA_CPI_CAM_CFG_LL_FRAMESLICE_LLY_BIT,UDMA_CPI_CAM_CFG_LL_FRAMESLICE_LLY_WIDTH);
    this->trace.msg("Updating low-left corner (x: %d, y: %d)\n", frameSliceLlx, frameSliceLly);
  }

  if (this->wroteUr) {
    this->wroteUr = false;
    this->frameSliceUrx = ARCHI_REG_FIELD_GET(ur,UDMA_CPI_CAM_CFG_UR_FRAMESLICE_URX_BIT,UDMA_CPI_CAM_CFG_UR_FRAMESLICE_URX_WIDTH);
    this->frameSliceUry = ARCHI_REG_FIELD_GET(ur,UDMA_CPI_CAM_CFG_UR_FRAMESLICE_URY_BIT,UDMA_CPI_CAM_CFG_UR_FRAMESLICE_URY_WIDTH);
    this->trace.msg("Updating up-right corner (x: %d, y: %d)\n", frameSliceUrx, frameSliceUry);
  }
  
  if (this->wroteSize) {
    this->wroteSize = false;
    this->rowLen = ARCHI_REG_FIELD_GET(size,UDMA_CPI_CAM_CFG_SIZE_ROWLEN_BIT,UDMA_CPI_CAM_CFG_SIZE_ROWLEN_WIDTH) + 1;
...
Cpi_rx_channel::Cpi_rx_channel(udma *top, Cpi_periph *periph, string name) : Udma_rx_channel(top, name), periph(periph)
{
}
