...
I2s_periph::I2s_periph(udma *top, int id, int itf_id) : Udma_periph(top, id)
{
  ...
  this->nb_tdm_channels = this->top->get_js_config()->get_child_int("**/i2s/tdm_channels");
...
void I2s_periph::reset(bool active)
{
  ...
  for (int i=0; i<this->nb_tdm_channels; i++)
  {
    this->tdm_channels[i]->reset(active);
  }
}
...
vp::io_req_status_e I2s_periph::custom_req(vp::io_req *req, uint64_t offset)
{
  ...
  {
    int channel_pair = (offset - 0x80) / 0x80;
    int channel = channel_pair*2 + ((offset % 0x80) >= 0x10);
    int channel_offset = offset & 0xf;

    if (channel < this->nb_tdm_channels)
    {
      return this->tdm_channels[channel]->req(req, channel_offset);
    }
  }
...
void I2s_rx_channel::handle_rx_bit(int sck, int ws, int bit)
{
  ...
  if (push)
  {
    ...
    if (this->periph->r_i2s_slv_setup.slave_words_get() == 0)
    {
      ((I2s_rx_channel *)this->periph->channel0)->push_data((uint8_t *)&result, bytes);
    }
    else
    {
      this->periph->tdm_channels[(this->periph->current_channel + this->periph->nb_tdm_channels - 1) % this->periph->nb_tdm_channels]->push_data((uint8_t *)&result, bytes);
    }
  }
}
