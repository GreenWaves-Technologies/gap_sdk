...
void Ne16::regfile_cxt() {
  ...
  for(auto addr=0; addr<NE16_NB_REG; addr++) {
    ...
    switch(addr) {
      ...
      case NE16_REG_PADDING0:
        this->padding_top_stride    = (value >> 28) & 0xf;
        this->padding_right_stride  = (value >> 24) & 0xf;
        this->padding_bottom_stride = (value >> 20) & 0xf;
        this->padding_left_stride   = (value >> 16) & 0xf;
        this->padding_value  = value & 0xffff;
        break;

      case NE16_REG_PADDING1:
        this->padding_top_nb    = (value >> 24) & 0xff;
        this->padding_right_nb  = (value >> 16) & 0xff;
        this->padding_bottom_nb = (value >>  8) & 0xff;
        this->padding_left_nb   = (value >>  0) & 0xff;
        break;

      case NE16_REG_PADDING2:
        this->padding_top_init    = (value >> 24) & 0xff;
        this->padding_right_init  = (value >> 16) & 0xff;
        this->padding_bottom_init = (value >>  8) & 0xff;
        this->padding_left_init   = (value >>  0) & 0xff;
        break;
      ...
      case NE16_REG_FILTER_MASK:
        this->filter_mask_map = value & 0x1ff;
        break;

      case NE16_REG_CONFIG0:
        // [31:29] reserved
        // [28] mode signed (0=inputs are uint8, 1=inputs are int8)
        this->mode_signed = ((value >> 28) & 0x1) ? true : false;
        // [27] bias broadcast (0=do not broadcast, 1=broadcast bias across Ko)
        this->bias_broadcast = ((value >> 27) & 0x1) ? true : false;
        // [26] scale broadcast (0=do not broadcast, 1=broadcast scale across Ko)
        this->scale_broadcast = ((value >> 26) & 0x1) ? true : false;
        ...
        // [11:9] Ki scatter (010=8-bits packing, 011=4-bits, 100=2-bits, 101=1-bit, rest=16-bits)
        this->ki_scatter = (value >> 9 & 0x7) == 0x2 ? 8 :
                           (value >> 9 & 0x7) == 0x3 ? 4 :
                           (value >> 9 & 0x7) == 0x4 ? 2 :
                           (value >> 9 & 0x7) == 0x5 ? 1 : 16;
...
void Ne16::printout() {
  ...
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_top_nb=%d\n", this->padding_top_nb); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_right_nb=%d\n", this->padding_right_nb); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_bottom_nb=%d\n", this->padding_bottom_nb); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_left_nb=%d\n", this->padding_left_nb); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_top_init=%d\n", this->padding_top_init); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_right_init=%d\n", this->padding_right_init); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_bottom_init=%d\n", this->padding_bottom_init); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_left_init=%d\n", this->padding_left_init); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_top_stride=%d\n", this->padding_top_stride); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_right_stride=%d\n", this->padding_right_stride); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_bottom_stride=%d\n", this->padding_bottom_stride); //int
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) padding_left_stride=%d\n", this->padding_left_stride); //int
  ...
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) filter_mask_map=%x\n", this->filter_mask_map); //int
  ...
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) mode_signed=%s\n", this->mode_signed ? "true" : "false"); //bool
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) ki_scatter=%s\n", this->ki_scatter ? "true" : "false"); //bool
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) bias_broadcast=%s\n", this->bias_broadcast ? "true" : "false"); //bool
  this->trace.msg(vp::trace::LEVEL_INFO, "(cfg) scale_broadcast=%s\n", this->scale_broadcast ? "true" : "false"); //bool
