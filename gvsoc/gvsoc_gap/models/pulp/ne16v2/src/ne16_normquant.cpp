...
int  Ne16::normquant_mult_cycle() {
  ...
  if(this->normalization_bits == 8) {
    ...
    for(auto i=0; i<nmult; i++) {
      auto ii = this->scale_broadcast ? 0 : i;
      for(auto col=0; col<this->NR_COLUMN; col++) {
        xt::view(this->accum, this->nq_iter*nmult+i, col) = xt::view(this->accum, this->nq_iter*nmult+i, col) * xt::view(nq, ii);
      }
    }
  }
    ...
    for(auto i=0; i<2; i++) {
      auto ii = this->scale_broadcast ? 0 : i;
      for(auto col=0; col<this->NR_COLUMN; col++) {
        xt::view(this->accum, this->nq_iter*nmult+i, col) = xt::view(this->accum, this->nq_iter*nmult+i, col) * xt::view(nq16, ii);
      }
    }
  }
...
int  Ne16::normquant_bias_cycle() {
  ...
  if(this->norm_option_bias) {
    ...
    for(auto col=0; col<this->NR_COLUMN; col++) {
      if(this->bias_broadcast) {
        xt::view(this->accum, xt::range(this->nqb_iter*8, (this->nqb_iter+1)*8), col) = (xt::view(this->accum, xt::range(this->nqb_iter*8, (this->nqb_iter+1)*8), col) + xt::view(nqb32, 0));
      }
      else {
