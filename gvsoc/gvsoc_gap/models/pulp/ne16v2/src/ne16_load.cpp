...
void Ne16::load_do_padding() { // not linear
  ...
  // update current top padding
  if(this->i_major == 0) {
    this->padding_top_current = this->padding_top_init;
  }
  else if(this->i_major == this->padding_top_nb) {
    this->padding_top_current = 0;
  }
  else {
    this->padding_top_current = this->padding_top_current + this->padding_top_stride;
  }

  // update current right padding
  if(this->j_major < this->subtile_nb_wo - this->padding_right_nb) {
    this->padding_right_current = 0;
  }
  else if(this->j_major == this->subtile_nb_wo - this->padding_right_nb) {
    this->padding_right_current = this->padding_right_init;
  }
  else {
    this->padding_right_current = this->padding_right_current + this->padding_right_stride;
  }

  // update current bottom padding
  if(this->i_major < this->subtile_nb_ho - this->padding_bottom_nb) {
    this->padding_bottom_current = 0;
  }
  else if(this->i_major == this->subtile_nb_ho - this->padding_bottom_nb) {
    this->padding_bottom_current = this->padding_bottom_init;
  }
  else {
    this->padding_bottom_current = this->padding_bottom_current + this->padding_bottom_stride;
  }

  // update current left padding
  if(this->j_major == 0) {
    this->padding_left_current = this->padding_left_init;
  }
  else if(this->j_major == this->padding_left_nb) {
    this->padding_left_current = 0;
  }
  else {
    this->padding_left_current = this->padding_left_current + this->padding_left_stride;
  }

  auto padding_top    = this->padding_top_current    > 5 ? 5 : this->padding_top_current    < 0 ? 0 : this->padding_top_current;
  auto padding_right  = this->padding_right_current  > 5 ? 5 : this->padding_right_current  < 0 ? 0 : this->padding_right_current;
  auto padding_bottom = this->padding_bottom_current > 5 ? 5 : this->padding_bottom_current < 0 ? 0 : this->padding_bottom_current;
  auto padding_left   = this->padding_left_current   > 5 ? 5 : this->padding_left_current   < 0 ? 0 : this->padding_left_current;

  // add explicit padding
  auto right_lim  = (5-padding_right  > this->w_size_in_hw) ? this->w_size_in_hw : 5-padding_right;
  auto bottom_lim = (5-padding_bottom > this->h_size_in_hw) ? this->h_size_in_hw : 5-padding_bottom;
  ...
  if(!this->mode16) {
    ...
    if(padding_left > 0 || padding_top > 0) {
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(0, padding_left), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
  
    // top
    if(padding_top > 0) {
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(padding_left, right_lim), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
    
    // top-right
    if((padding_right > 0 || padding_top > 0 ) && (right_lim <= this->w_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(right_lim, w_size_in_hw), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
  
    // right
    if((padding_right > 0) && (right_lim <= this->w_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(padding_top, bottom_lim), xt::range(right_lim, this->w_size_in_hw), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
  
    // bottom-right
    if((padding_right > 0 || padding_bottom > 0) && (right_lim <= this->w_size_in_hw) && (bottom_lim <= this->h_size_in_hw)) {
    ...
    // bottom
    if((padding_bottom > 0) && (bottom_lim <= this->h_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(bottom_lim, this->h_size_in_hw), xt::range(padding_left, right_lim), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
  
    // bottom-left
    if((padding_left > 0 || padding_bottom > 0) && (bottom_lim <= this->h_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(bottom_lim, this->h_size_in_hw), xt::range(0, padding_left), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
  
    // left
    if(padding_left > 0) {
      xt::view(this->x_buffer, xt::range(padding_top, bottom_lim), xt::range(0, padding_left), xt::range(0, this->load_k_in_lim)) = this->padding_value;
    }
    ...
    // top-left
    if(padding_left  > 0 || padding_top > 0) {
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(0, padding_left), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(0, padding_left), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
  
    // top
    if(padding_top  > 0) {
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(padding_left, right_lim), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(padding_left, right_lim), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
    
    // top-right
    if((padding_right > 0 || padding_top > 0) && (right_lim <= this->w_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(right_lim, w_size_in_hw), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(0, padding_top), xt::range(right_lim, w_size_in_hw), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
  
    // right
    if((padding_right > 0) && (right_lim <= this->w_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(padding_top, bottom_lim), xt::range(right_lim, this->w_size_in_hw), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(padding_top, bottom_lim), xt::range(right_lim, this->w_size_in_hw), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
    ...
    // bottom
    if((padding_bottom > 0) && (bottom_lim <= this->h_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(bottom_lim, this->h_size_in_hw), xt::range(padding_left, right_lim), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(bottom_lim, this->h_size_in_hw), xt::range(padding_left, right_lim), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
  
    // bottom-left
    if((padding_left > 0 || padding_bottom > 0) && (bottom_lim <= this->h_size_in_hw)) {
      xt::view(this->x_buffer, xt::range(bottom_lim, this->h_size_in_hw), xt::range(0, padding_left), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(bottom_lim, this->h_size_in_hw), xt::range(0, padding_left), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
  
    // left
    if(padding_left > 0) {
      xt::view(this->x_buffer, xt::range(padding_top, bottom_lim), xt::range(0, padding_left), xt::range(0, this->load_k_in_lim,   2)) = this->padding_value & 0xff;
      xt::view(this->x_buffer, xt::range(padding_top, bottom_lim), xt::range(0, padding_left), xt::range(1, this->load_k_in_lim+1, 2)) = this->padding_value >> 8;
    }
...
void Ne16::load_filter_masking() {
  ...
  for(auto i=0; i<this->fs*this->fs; i++) {
    if(this->filter_mask_map >> i & 0x1) {
      xt::view(W_mask, i) = 0;
    }
  }
