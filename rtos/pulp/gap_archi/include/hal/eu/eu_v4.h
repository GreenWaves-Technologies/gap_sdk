...
#ifndef __HAL_EU_EU_V4_H__
#define __HAL_EU_EU_V4_H__
...
{
  IP_WRITE_PTR(ARCHI_EU_DEMUX_ADDR, EU_SW_EVENTS_DEMUX_OFFSET + EU_CORE_TRIGG_SW_EVENT + (event << 2), coreSet);
}
...
static inline unsigned int eu_sem_addr(int sem_id)
{
  return ARCHI_EU_DEMUX_ADDR + EU_SEM_DEMUX_OFFSET + EU_SEM_AREA_OFFSET_GET(sem_id);
}
 
static inline int eu_sem_id(unsigned int sem_addr)
{
  return EU_SEM_AREA_SEMID_GET(sem_addr);
}
 
static inline void eu_sem_inc(unsigned int sem_addr, uint8_t value)
{
    IP_WRITE_PTR(sem_addr, EU_HW_SEM_COUNTER, (uint32_t)value);
}
 
static inline uint32_t eu_sem_dec(unsigned int sem_addr)
{
...
static inline unsigned int eu_bitfield_addr(int bitfield_id)
{
  return ARCHI_EU_DEMUX_ADDR + EU_BITFIELD_DEMUX_OFFSET + EU_BITFIELD_AREA_OFFSET_GET(bitfield_id);
}

static inline int eu_bitfield_id(unsigned int bitfield_addr)
{
...
static inline void eu_bitfield_set_from_id(int id, uint32_t value)
{
    IP_WRITE_PTR(ARCHI_EU_DEMUX_ADDR, EU_BITFIELD_DEMUX_OFFSET + EU_HW_BITFIELD_SET + (id << EU_BITFIELD_SIZE_LOG2), value);
}

static inline void eu_bitfield_clear_from_id( int id, uint32_t value)
{
