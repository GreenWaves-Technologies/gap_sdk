...
PMU_StateT PI_L2 PMUState = {0, 0, {0, 0, 0, 0}, {0, 0}};
PMU_RetentionStateT PI_L2 PMURetentionState;

unsigned int PI_L2 FllsFrequency[N_FLL];
unsigned int __rt_wakeup_use_fast;


static unsigned char PI_L2 SystemStateToSCUFastSeq[PMU_LAST_STATE+1] = {
  [SOC_HP] = 2,
  [SOC_LP] = 3,
  [SOC_CLUSTER_HP] = 2,
  [SOC_CLUSTER_LP] = 3,
...
static unsigned char PI_L2 RetPMUStateToPMUState[] = {
        SOC_HP,         /* 0: SOC_HP */
        SOC_LP,         /* 1: SOC_LP */
...
static unsigned char PI_L2 ToHWDCDC_Pos[] = {
...
static int __rt_socevents_status[4];

static int __rt_periph_get_event(int event)
{
    return 0;
}

static void __rt_periph_wait_event(int a, int b)
{
...
{
    InitOneFll(fll, PMURetentionState.Fields.FllSoCRetention);
    pos_freq_domains[fll] = FllsFrequency[fll];
  return pos_freq_domains[fll];
}

unsigned int pos_fll_set_freq(int fll, unsigned int frequency)
{
  pos_freq_domains[fll] = SetFllFrequency(fll, frequency, 0);
  return pos_freq_domains[fll];
}