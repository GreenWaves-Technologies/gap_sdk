...
int __pi_spi_open(struct spim_cs_data **cs_data, struct pi_spi_conf *conf)
{
    ...
    else
    {
        ...
        hal_soc_eu_clear_fc_mask(SOC_EVENT_UDMA_SPIM_TX(conf->itf));
        hal_soc_eu_clear_fc_mask(SOC_EVENT_UDMA_SPIM_RX(conf->itf));

        __g_spim_drv_data[conf->itf]=pi_data_malloc(sizeof(struct spim_driver_data));
...
int __pi_spi_close(struct spim_cs_data *cs_data)
{
    ...
    if(drv_data->nb_open == 0)
    {
        ...
        pi_data_free(drv_data,sizeof(drv_data));
        return 0;
    }
        ...
        pi_task_t *task)
{
    ...
    else
    {
        struct spim_transfer transfer;
        task->data[0] = (uintptr_t)cs_data;
        task->data[1] = (uintptr_t)data;
        task->data[2] = (uintptr_t)len;
        task->data[3] = (uintptr_t)flags;
        task->data[4] = (uintptr_t)task;
        task->data[5] = (uintptr_t)SPI_TRANSFER_TYPE_COPY;
        task->data[6] = (uintptr_t)addr;
        //__pi_spim_drv_fifo_enqueue(cs_data, &transfer, task);
        __pi_spim_drv_fifo_enqueue_task(cs_data, task);
    }
    restore_irq(irq);
