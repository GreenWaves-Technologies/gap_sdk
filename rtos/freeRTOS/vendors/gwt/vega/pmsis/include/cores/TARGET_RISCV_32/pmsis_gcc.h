/**************************************************************************//**
 * @file     pmsis_gcc.h
 * @brief    PMSIS  compiler GCC header file
 * @version  V6.0.1 - GAP9 support
 ...
 */
/*
   Modifications copyright (C) 2020 GreenWaves Technologies
...
/**
  \brief   Enable EBREAKM core stall
  \details Enables EBREAKM core stall by setting the ebreakm bit in the DCSR.
           Can only be executed in Privileged modes.
 */
__attribute__((always_inline)) __STATIC_INLINE void __enable_ebreakm(void)
{
    int dcsr = 0x1UL << 15;
    asm volatile ("csrrs %0, 0x7B0, %1" : "=r" (dcsr) : "r" (dcsr) );
    dcsr = 0x1UL << 12;
    asm volatile ("csrrs %0, 0x7B0, %1" : "=r" (dcsr) : "r" (dcsr) );
}
...
/**
  \brief   Disable EBREAKM core stall
  \details Disables EBREAKM core stall by clearing the ebreakm bit in the DCSR.
           Can only be executed in Privileged modes.
 */
__attribute__((always_inline)) __STATIC_INLINE void __disable_ebreakm(void)
{
    int dcsr = 0x1UL<<15;
    asm volatile ("csrrc %0, 0x7B0, %1" : "=r" (dcsr) : "r" (dcsr) );
}
