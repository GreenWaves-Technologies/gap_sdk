...
//TODO undo the override
#ifndef ALLOC_TRACE
#define ALLOC_TRACE(...)
//#define ALLOC_TRACE(level, x...)  printf(x)
#endif

#ifdef __FREERTOS__

void pos_alloc_power_ctrl(uint32_t banks, uint32_t power, uint32_t standby)
{
#if !defined(CONFIG_GAP9_5) && !defined(__GAP9_5__)

    if (power)
    {
        uint32_t l2_ctrl_active = apb_soc_ctrl_l2_ctrl_active_get(ARCHI_APB_SOC_CTRL_ADDR);
        uint32_t l2_pwr_active = apb_soc_ctrl_l2_pwr_active_get(ARCHI_APB_SOC_CTRL_ADDR);
        uint32_t mask = (banks << 16) | (banks << 0);

        // First close power Switches
        l2_pwr_active &= ~mask;
        apb_soc_ctrl_l2_pwr_active_set(ARCHI_APB_SOC_CTRL_ADDR,  l2_pwr_active);

        // Then connect memories
        l2_ctrl_active &= ~mask;
        apb_soc_ctrl_l2_ctrl_active_set(ARCHI_APB_SOC_CTRL_ADDR, l2_ctrl_active);

        // Wait until we get the ack to not access a bank which is not yet powered-up
        while(apb_soc_ctrl_l2_ack_get(ARCHI_APB_SOC_CTRL_ADDR) & mask);
    }
    else
    {
        uint32_t l2_ctrl_active = apb_soc_ctrl_l2_ctrl_active_get(ARCHI_APB_SOC_CTRL_ADDR);
        ...
        // The open Power Switches
        if (standby)
        {
            // First isolate memory from control pins
            l2_ctrl_active |= (banks << 0);
            apb_soc_ctrl_l2_ctrl_active_set(ARCHI_APB_SOC_CTRL_ADDR, l2_ctrl_active);

            // No need to control the memory, it will go to user controlled retention
            // mode when we cut the clock
        }
        else
        {
            // First isolate memory from control pins
            l2_ctrl_active |= (banks << 16);
            apb_soc_ctrl_l2_ctrl_active_set(ARCHI_APB_SOC_CTRL_ADDR, l2_ctrl_active);

            // And shutdown the array
            l2_pwr_active |= (banks << 16) | (banks << 0);
            apb_soc_ctrl_l2_pwr_active_set(ARCHI_APB_SOC_CTRL_ADDR,  l2_pwr_active);
        }
    }
#endif
