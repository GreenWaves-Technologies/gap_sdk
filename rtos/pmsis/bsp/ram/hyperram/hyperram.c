...
#if defined(__GAP8__) || defined(__PULP_OS__)
  uint32_t start_addr = 0;
#else
  uint32_t start_addr = pi_hyper_offset(&hyperram->hyper_device);
#endif
...
#else
  pi_hyper_copy_async(&hyperram->hyper_device, addr, data, size, ext2loc, task);
#endif
}
...
#else
  pi_hyper_copy_2d_async(&hyperram->hyper_device, addr, data, size, stride, length, ext2loc, task);
#endif
}
