...
static int32_t aps25xxxn_ioctl(struct pi_device *device, uint32_t cmd, void *arg)
{
    switch (cmd)
    {
        case PI_RAM_IOCTL_INFO:
            {
                aps25xxxn_t *aps25xxxn = (aps25xxxn_t *)device->data;
                struct pi_ram_info *ram_info = (struct pi_ram_info *)arg;
                // TODO find a way to know what is on the flash, as they may be a boot binary
                ram_info->size = CONFIG_APS25XXXN_SIZE;
                return (int32_t)ram_info;
            }
        case PI_RAM_IOCTL_AES_ENABLE:
            {
                aps25xxxn_t *aps25xxxn = (aps25xxxn_t *)device->data;
                return pi_octospi_ioctl(&aps25xxxn->octospi_device, PI_OCTOSPI_IOCTL_ENABLE_AES, (void*) arg);
            }
            break;
        case PI_RAM_IOCTL_SET_MBA:
            {
                aps25xxxn_t *aps25xxxn = (aps25xxxn_t *)device->data;
                return pi_octospi_ioctl(&aps25xxxn->octospi_device, PI_OCTOSPI_IOCTL_MBA, (void*) arg);
            }
            break;
        default:
            return -1;
