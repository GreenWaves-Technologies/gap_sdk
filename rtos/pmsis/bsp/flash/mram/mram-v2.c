...
#ifndef CONFIG_XIP_MRAM
        pi_pmu_domain_state_change(PI_PMU_DOMAIN_MRAM, PI_PMU_DOMAIN_STATE_ON, 0);
#endif
...
void pi_mram_sa_trim_config(struct pi_device *device, u_int8_t sense_amplifier_trim)
{
    ...
    uint32_t *trim_cfg_buff = trim_cfg_buffer;
    unsigned int base = mram->base;
    ...
    trim_cfg_buff[1] = 0xFD798D00;
    trim_cfg_buff[2] = 0x620490D0;
    trim_cfg_buff[3] = 0x0406082E; // 127:96 (507 is 406)
    trim_cfg_buff[4] = 0x0400001B; // Bits (153:148) 25:20 are the sense amplifier bits
    trim_cfg_buff[5] = 0x00000000;
    trim_cfg_buff[6] = 0x00000000;

    // Set the sense amplifier value
    trim_cfg_buff[4] = trim_cfg_buff[4] | (sense_amplifier_trim << 20);

    mram->pending_copy = pi_task_block(&task);
    __rt_mram_trim_cfg_exec(mram, trim_cfg_buff, 0, TRIM_CFG_SIZE*4);
    pi_task_wait_on(&task);
    // tRH: End of configuration to any operation
    pi_time_wait_us(20);
}
...
void pi_open_access_to_nvr(struct pi_device *device)
{
    ...
    udma_mram_mode_t mode = { .raw = udma_mram_mode_get(base) };
    udma_mram_mode_set(base, mode.raw |
            UDMA_MRAM_MODE_TMEN(1) |
            UDMA_MRAM_MODE_NVR(1)
    );
}
...
void pi_close_access_to_nvr(struct pi_device *device)
{
    ...
    mode.raw = udma_mram_mode_get(base);
    udma_mram_mode_set(base, mode.raw &
            ~(UDMA_MRAM_MODE_TMEN(1))
    );
}
...
#ifndef CONFIG_XIP_MRAM
        pi_pmu_domain_state_change(PI_PMU_DOMAIN_MRAM, PI_PMU_DOMAIN_STATE_OFF, 0);
#endif
...
PI_LOCAL_CODE static void mram_erase_sector_async(struct pi_device *device, uint32_t addr, pi_task_t *task)
{
    ...
    if (likely(!mram->pending_copy))
    {
        ...
        udma_mram_erase_size_set(base, 0); /* as usual in hardware 0 means 1 */
        udma_mram_trans_cfg_set(base, UDMA_MRAM_TRANS_CFG_VALID(1));
