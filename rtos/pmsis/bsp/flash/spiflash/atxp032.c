...
#define ATXP032_READ_STATUS_CMD (0x65 | PI_OCTOSPI_CMD_ADDR_EVEN)
...
#define ATXP032_WRITE_STATUS_CMD (0x71 | PI_OCTOSPI_CMD_ADDR_EVEN)
...
#define ATXP032_WRITE_ENABLE_CMD (0x6 | PI_OCTOSPI_CMD_ADDR_EVEN)
...
#define ATXP032_ERASE_CMD (0x20 | PI_OCTOSPI_CMD_ADDR_EVEN)
...
#define ATXP032_PROGRAM_CMD (0x02 | PI_OCTOSPI_CMD_ADDR_EVEN)
...
static uint32_t atxp032_get_status(atxp032_t *atxp032)
{
    ...
    int prev = pi_octospi_ioctl(&atxp032->octospi_device, PI_OCTOSPI_IOCTL_ENABLE_AES, (void*) 0);
#endif
  uint32_t data;
  pi_octospi_op_conf_t op = { .cmd=ATXP032_READ_STATUS_CMD, .latency=ATXP032_READ_STATUS_LATENCY_OCTO, .flags=ATXP032_READ_STATUS_FLAGS_OCTO };
...
#if defined(__GAP9__)
    /* disable AES while writing to configuration registers */
    pi_octospi_ioctl(&atxp032->octospi_device, PI_OCTOSPI_IOCTL_ENABLE_AES, (void*) prev);
#endif
  return data;
}
...
static int atxp032_open(struct pi_device *device)
{
  ...
  octospi_conf.aes_conf = &(conf->flash.aes_conf);
  octospi_conf.mba = conf->mba;
...
#if defined(__GAP9__)
    /* disable AES while writing to configuration registers */
    pi_octospi_ioctl(&atxp032->octospi_device, PI_OCTOSPI_IOCTL_ENABLE_AES, (void*) 1);
#endif
...
static int32_t atxp032_ioctl(struct pi_device *device, uint32_t cmd, void *arg)
{
  switch (cmd)
  {
    case PI_FLASH_IOCTL_INFO:
    {
        ...
        flash_info->size = CONFIG_ATXP032_SIZE;
        break;
    }
    case PI_FLASH_IOCTL_AES_ENABLE:
    {
        atxp032_t *atxp032 = (atxp032_t *)device->data;
        pi_octospi_ioctl(&atxp032->octospi_device, PI_OCTOSPI_IOCTL_ENABLE_AES, (void*) arg);
        break;
    }
    case PI_FLASH_IOCTL_SET_MBA:
    {
        atxp032_t *atxp032 = (atxp032_t *)device->data;
        pi_octospi_ioctl(&atxp032->octospi_device, PI_OCTOSPI_IOCTL_MBA, (void*) arg);
        break;
    }
  }
  return 0;
