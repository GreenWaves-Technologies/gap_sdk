...
#define CONFIG_m24c02_BURST_SIZE 16

// Delay to wait after a write burst is done before doing the next one
#define CONFIG_m24c02_FLUSH_DELAY_US 10
...
static void bsp_m24c02_check_pending_write(eeprom_t *eeprom);


int pi_m24c02_open(struct pi_device *device)
{
    struct pi_m24c02_conf *conf = (struct pi_m24c02_conf *)device->config;

    if (bsp_m24c02_open(conf))
        return -1;
...
static void pi_m24c02_close(struct pi_device *device)
{
...
static void bsp_m24c02_check_end_of_flush(void *arg)
{
    eeprom_t *eeprom = arg;
...
static void bsp_m24c02_check_end_of_write(void *arg)
{
    ...
    pi_task_push_delayed_us(pi_task_callback(&eeprom->internal_task, bsp_m24c02_check_end_of_flush, (void *)eeprom), CONFIG_m24c02_FLUSH_DELAY_US);
}
...
static void bsp_m24c02_check_pending_write(eeprom_t *eeprom)
{
    ...
    if (!eeprom->waiting_page_flush)
    {
        ...
        int iter_size = CONFIG_m24c02_BURST_SIZE - (eeprom->pending_write_addr & (CONFIG_m24c02_BURST_SIZE - 1));

        if (iter_size > eeprom->pending_write_size)
            iter_size = eeprom->pending_write_size;

        // The address is store big-endian, swap it now
        uint32_t addr = eeprom->pending_write_addr & 0xFF;
        ...
        pi_i2c_write_dual_async(&eeprom->i2c_device, &addr, (void *)data, 1, iter_size, pi_task_callback(&eeprom->internal_task, bsp_m24c02_check_end_of_write, (void *)eeprom));
    }
}


static void bsp_m24c02_enqueue_copy(eeprom_t *eeprom, uint32_t eeprom_addr, void *data, uint32_t size, pi_task_t *task, int is_write)
{
    ...
    if (!eeprom->pending_task)
    {
        ...
        else
        {
            ...
            uint32_t addr = eeprom_addr & 0xFF;
            pi_i2c_write_read_async(&eeprom->i2c_device, &addr, data, 1, size, task);
        }
    }
    else
    {
...
static void pi_m24c02_read_async(struct pi_device *device, uint32_t eeprom_addr, void *data, uint32_t size, pi_task_t *task)
{
    bsp_m24c02_enqueue_copy(device->data, eeprom_addr, data, size, task, 0);
}


static void pi_m24c02_write_async(struct pi_device *device, uint32_t eeprom_addr, void *data, uint32_t size, pi_task_t *task)
{
    bsp_m24c02_enqueue_copy(device->data, eeprom_addr, data, size, task, 1);
}


static pi_eeprom_api_t pi_m24c02_api =
{
  .open                 = &pi_m24c02_open,
  .close                = &pi_m24c02_close,
  .read_async           = &pi_m24c02_read_async,
  .write_async          = &pi_m24c02_write_async
};


void pi_m24c02_conf_init(struct pi_m24c02_conf *conf)
{
  conf->eeprom.api = &pi_m24c02_api;
  bsp_m24c02_conf_init(conf);
}
