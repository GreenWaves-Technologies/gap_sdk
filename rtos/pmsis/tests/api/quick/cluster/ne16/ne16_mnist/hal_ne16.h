...
#ifdef __FREERTOS__
#define NE16_BARRIER_NOSTATUS()      hal_eu_evt_mask_wait_and_clr (1 << NE16_EVT0)
#define NE16_BARRIER()               do { hal_eu_evt_mask_wait_and_clr (1 << NE16_EVT0); } while((*(int volatile *)(NE16_ADDR_BASE + NE16_STATUS)) != 0)
...
#define NE16_BARRIER_ACQUIRE(job_id) job_id = NE16_READ_CMD(job_id, NE16_ACQUIRE); \
                                     while(job_id < 0) { hal_eu_evt_mask_wait_and_clr (1 << NE16_EVT0); NE16_READ_CMD(job_id, NE16_ACQUIRE); };
#else
