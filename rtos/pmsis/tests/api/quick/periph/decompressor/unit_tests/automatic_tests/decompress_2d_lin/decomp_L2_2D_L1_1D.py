...
# Creates a golden model that decompresses data from an 2D L2 memory
# to an L1 1D memory
...
def output_to_file():
    ...
    file.write('/* ------------------------------------------ */\n')
    file.write('static PI_L1 u' + word_size + ' tcdm_expect_data[' + str(transfer_size) + '] = {\n')

    # Decompress in byte size
    if (word_length_choice == 0):
        for i in range(0, transfer_size):
    ...
    # Decompress in half word size
        ...
        for i in range(0, transfer_size):
            ...
            half_word = hex(half_word)[2:].zfill(4)
    ...
    # Writes the configuration for the decompressor command
    file.write("/* ------------------------------ */\n")
    ...
    file.write("    .transf_type            = PI_CL_DMA_DECOMPRESSOR_TRANSF_EXT_2D_LOC_LIN,\n"),
    ...
    file.write("    .l2_2d.count            = {},\n".format(item_count - 1)),
    file.write("    .l2_2d.stride           = {},\n".format(stride_length - 1)),
    file.write("    .tcdm_2d.count          = 0,\n"),
    file.write("    .tcdm_2d.stride         = 0,\n"),
    file.write("    .special_symbol         = {},\n".format(special_symbol)),
    file.write("    .done                   = 0,\n"),
    file.write("    .flag.update_lut        = 0,\n"),
    file.write("    .flag.update_2d_ext     = 1,\n"),
    file.write("    .flag.update_2d_loc     = 0,\n"),
...
# Initialize random with fixed seed to get same results every time
random.seed(30)
...
stride_index = ((args.start_byte * 8) + args.start_bit)
...
while (elements_remaining > 0):
    ...
    # Leap using stride length to the next element
    stride_index = stride_index + (stride_length * qbit)
    # Divide by 32 to get next item to point to (because each item in L2 = 32 bits)
    item_index = stride_index // 32

    item_to_decomp = randomlist[item_index]
    item_to_decomp = bin(int(item_to_decomp, 16))[2:].zfill(bin_zero_fill)
    start_bit_index = stride_index % 32
    end_bit_index = start_bit_index + qbit
