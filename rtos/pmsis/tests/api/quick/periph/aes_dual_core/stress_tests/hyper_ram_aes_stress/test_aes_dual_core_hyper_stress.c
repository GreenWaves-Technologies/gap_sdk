...
#define HYPERRAM_MODEL_MAX_BAUDRATE Mhz(160)
...
int test_configuration(pi_aes_utils_conf_t* aes_conf)
{
    ...
    conf.baudrate = HYPERRAM_MODEL_MAX_BAUDRATE;
...
int test_entry()
{
    ...
    size_t freqs_size = sizeof(freqs)/sizeof(freqs[0]);
    for (size_t i = 0; i < freqs_size; i++)
    {
        pi_freq_set(PI_FREQ_DOMAIN_FC,    freqs[i].fc_freq);
        pi_freq_set(PI_FREQ_DOMAIN_PERIPH,freqs[i].periph_freq);
        printf("Frequency: FC=%dHz, PERIPH=%dHz\n",
