...
PI_L2 uint32_t timeout_us[NB_ITER] = {3000,   /* OK */
                                      0,      /* OK */
                                      10,     /* KO */
                                      1000000 /* OK */
};
PI_L2 char hello_rx[20] = {0};
...
int32_t __timeout_async_exec(uint32_t iter)
{
    ...
    PRINTF("Enqueueing RX with timeout=%ldus\n", timeout);
    pi_task_block(wait_task);
    pi_task_timeout_set(wait_task, timeout);
    pi_uart_read_async(&uart, hello_rx, hello_tx_len, wait_task);
    pi_uart_write(&uart, hello_tx, hello_tx_len);
...
int32_t test_timeout_sync(void)
{
    ...
    for (uint32_t iter=0; iter<(uint32_t) NB_ITER; iter++)
    {
        ...
        PRINTF("Sending RX with timeout=%ldus, iter=%ld\n", timeout, iter);
        pi_task_block(&task_tx);
        pi_uart_write_async(&uart, hello_tx, hello_tx_len, &task_tx);
        status = pi_uart_read_timeout(&uart, hello_rx, hello_tx_len, timeout);
        errors += (status != 0);
        #if !defined(NO_PRINTF)
        if (status)
        {
