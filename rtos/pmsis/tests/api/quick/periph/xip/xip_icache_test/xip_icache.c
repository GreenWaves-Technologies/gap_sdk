...
#ifdef CLUSTER_FLUSH
    // Now test flush --> guinea_pig_xip_long will be replaced with a bunch of nops
    //for(volatile int i = 0; i < 7; i++)
    //{
    //    ((volatile uint16_t*)guinea_pig_xip_long)[i] = 0x1;
    //}
    guinea_pig_xip_long();
    if(g_guinea_pig[2] !=1)
    {
        printf("xip icache not flushed\n");
        errors++;
    }
#endif
...
void helloworld(void)
{
...
#ifdef CLUSTER_FLUSH
    // set xip cluster enable flag for cluster icache flush
    pi_xip_cluster_is_on_set(1);
#endif
    ...
    for(volatile int i = 0; i < 7; i++)
    {
        ((uint16_t*)guinea_pig_xip_long)[i] = 0x1;
    }
