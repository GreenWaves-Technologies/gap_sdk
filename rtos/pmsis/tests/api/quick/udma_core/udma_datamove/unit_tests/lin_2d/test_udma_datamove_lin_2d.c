...
static void test_lin_2d(void)
{
    ...
    for (size_t j = 0; j < NB_DEVICES; j++)
    {
        ...
        conf[j].src_trf_cfg.type = PI_UDMA_DATAMOVE_TRF_LINEAR;
    ...
    for (size_t j = 0; j < NB_DEVICES; j++)
    {
        ...
        for (size_t k = 0; k < DATA_SIZE; k++)
        {
            ...
            if (src_data[j][k] != dst_data[j][index_2d])
            {
                printf("ERROR(p%d-#%d): expected %x, got %x\n", j, k, src_data[j][k], dst_data[j][index_2d]);
                pmsis_exit(-1);
            }
            row_counter++;
        }
...
int main(void)
{
    printf("\n\n\t *** PMSIS UDMA DATAMOVE LIN/2D Tests ***\n\n");
    return pmsis_kickoff((void *) test_lin_2d);
}
