...
def create_inference_graph(wanted_words, sample_rate, clip_duration_ms,
                           ...
                           dct_coefficient_count, model_architecture, model_size_info, preprocess):
  ...
  if FLAGS.freeze_bn_folded_model:
    logits = quant_models.create_model(
        fingerprint_input, model_settings, model_architecture, model_size_info, is_training=False, runtime_settings=runtime_settings)
  else:
...
def main(_):
  ...
  input_tensor, output_tensor = create_inference_graph(
      ...
      FLAGS.dct_coefficient_count, FLAGS.model_architecture, FLAGS.model_size_info, FLAGS.preprocess)
  if FLAGS.quantize:
    tf.contrib.quantize.experimental_create_eval_graph(symmetric=True)
  models.load_variables_from_checkpoint(sess, FLAGS.start_checkpoint)

  # Turn all the variables into inline constants inside the graph and save it.
  frozen_graph_def = graph_util.convert_variables_to_constants(
      sess, sess.graph_def, ['identity_out'])
