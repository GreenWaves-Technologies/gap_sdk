...
def run(dataset_dir, small_object_area_threshold, foreground_class_of_interest, download=True, coco_dir=None):
  ...
  if not coco_dir:
    ...
    if not tf.io.gfile.isdir(coco_dir):
      tf.compat.v1.logging.info('Making dir:', coco_dir)
      tf.io.gfile.makedirs(coco_dir)

  if download:
    tf.compat.v1.logging.info('Downloading COCO dataset...')
...
#print(FLAGS.coco_dirname)
#run('dataset_dir', 0.05, ['bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat'])
