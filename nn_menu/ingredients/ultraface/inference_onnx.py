...
import argparse
import numpy as np
from evaluation.box_utils import predict

# ------------------------------------------------------------------------------------------------------------------------------------------------
# Face detection using UltraFace-320 onnx model
face_detector_onnx = "models/model.onnx"
model = onnx.load(face_detector_onnx)
...
for i in range(boxes.shape[0]):
    ...
    orig_image = cv2.rectangle(orig_image, (box[0], box[1]), (box[2], box[3]), color, 2)
