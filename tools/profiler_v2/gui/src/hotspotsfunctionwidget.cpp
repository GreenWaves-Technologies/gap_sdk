...
const char* HotspotsFunctionWidget::columnHeader[] = {
    "Time %",
    "Total Time",
    "Function name",
    "From file",
...
const int HotspotsFunctionWidget::nbColumn =5;
/*const int HotspotsFunctionWidget::nbColumn = \
    sizeof(HotspotsFunctionWidget::columnHeader) / sizeof(HotspotsFunctionWidget::columnHeader[0]);
*/

HotspotsFunctionWidget::HotspotsFunctionWidget(QWidget* parent,
...

{
    ...
    //asmCode->setWordWrapMode(QTextOption::NoWrap);
    sourceCodeWrap->sourceCode->setReadOnly(false);
    //asmCode->setReadOnly(true);
...
void HotspotsFunctionWidget::getHotspots(uint64_t t0Select, uint64_t t1Select){
    // Getting hotspots for a time range selected by the user
    //assert(backend);
    //backend->getBackend()->get_hotspots(this, -1, t0Select, t1Select);
}

void HotspotsFunctionWidget::getHotspots() {
    //assert(backend);
    //qDebug() << "HotspotsFunctionWidget::getHotSpots Null, -1, 0, 60000000000";
    //backend->getBackend()->get_hotspots(this, -1, 0, 60000000000);
    //qDebug() << "Done getHotSpots Null, -1, 0, 60000000000";
}

/*
void HotspotsFunctionWidget::switchLegendMode(LegendMode newMode)
{
...
/*void HotspotsFunctionWidget::updateTimeStamps(void)
{
  ...
  for (uint i = 0for (auto x: beAllHotspots)
        allHotspots.push_back(SafeHotspot(x));; i < data.size(); i++){
...
QString HotspotsFunctionWidget::formatTimeStamp(QString ts) const {
...
void HotspotsFunctionWidget::removeTableRows(){
    for(int i=0; i<table->rowCount(); i++)
        table->removeRow(i);
}

void HotspotsFunctionWidget::updateHotspotsTable(Hospot_results *ht_result,
                                                 ...
                                                 int64_t ht_max_timestamp)
{
    ...
    ht_result->get_function_hotspots(beHotspots);
    ht_result->get_line_hotspots(beAllHotspots);
    for (auto x: beHotspots)
        hotspots.push_back(SafeHotspotFunctionInfo(x));
...
void HotspotsFunctionWidget::fillHotspotsTable()

{
    ...
    for (auto x: hotspots)
    {
        ...
        table->setItem(z, 2, new QTableWidgetItem(QString::fromStdString(x.funcName)));

        table->setItem(z, 3, new QTableWidgetItem(QString::fromStdString(x.fileName)));

        QTableWidgetItem* itLine =  new QTableWidgetItem(QString::number(x.line));
        table->setItem(z, 4, itLine);
...
void HotspotsFunctionWidget::exportTableToTextFile(const char* filename) const {
...
void HotspotsFunctionWidget::selectFunction(int row){


    QTableWidgetItem* funcItem = table->item(row, 2);
    QTableWidgetItem* fileItem = table->item(row, 3);
    QTableWidgetItem* lineItem = table->item(row, 4);
    if (funcItem == nullptr){
...
void HotspotsFunctionWidget::selectFunction(QTableWidgetItem* it){
    selectFunction(it->row());
}

void HotspotsFunctionWidget::selectFunction(std::string funcName,
                                            std::string fileName,
                                            int hotspotLine){
...
std::string HotspotsFunctionWidget::read_file(std::string filename){
...
std::string HotspotsFunctionWidget::get_source_code(std::string funcName,
                                                    std::string fileName,
                                                    int hotspotLine) {
...
void HotspotsFunctionWidget::getFileHotspotList(const std::string & fileName,
...
void HotspotsFunctionWidget::setSourceCode(std::string funcName,
                                           std::string fileName,
                                           int hotspotLine){
...
void HotspotsFunctionWidget::selectRow(const char* name){
...
void HotspotsFunctionWidget::handleVisibilityChanged(bool visible){
