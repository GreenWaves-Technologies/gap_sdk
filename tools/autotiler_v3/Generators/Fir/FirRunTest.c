...
L1_CL_MEM short int * __restrict__ Coeffs;
...
static void cluster_main()
{
	...
	rt_perf_start(&perf);
#endif
	TiledFir10GenericScalar(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 10, "GenericScalar");
	TiledFir10Generic(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 10, "GenericVect  ");
	TiledFir10Opt(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 10, "OptVect      ");
	TiledFir20GenericScalar(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 20, "GenericScalar");
	TiledFir20Generic(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 20, "GenericVect  ");
	TiledFir20Opt(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 20, "OptVect      ");
	TiledFir40GenericScalar(Samples, Coeffs, Out, 10);
	DumpPerf(&perf, 40, "GenericScalar");
	TiledFir40Generic(Samples, Coeffs, Out, 10);
