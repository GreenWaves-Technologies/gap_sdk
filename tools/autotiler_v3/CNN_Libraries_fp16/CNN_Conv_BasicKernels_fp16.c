...
/* Padded Convolution Border processing
	...
	Input feature maps, Output feature maps and filter on fp16
		...
		KerConvNxNStrideS_Border_fp16		NxN convolution, stride S, Left, Right, Top and Bottom borders

		KerConvNxMStrideSxSy_Border_fp16	NxM convolution, stride Sx,Sy, Left, Right, Top and Bottom borders

		KerConvNxMDxDyStrideSxSy_Border_fp16	NxM convolution, dilation Dx,Dy, stride Sx,Sy, Left, Right, Top and Bottom borders
*/
	...
	)
{
	...
	for (unsigned int i=Ho_F; i<Ho_L; i++) {
		F16V A = *((F16V *)PtIn) * C; PtIn += W;
		*PtOut =  *PtOut+A[0]+A[1]; PtOut+=Wo;
	}
}
...

{
	...
	if (PadL) KerConv2x1from3x1StrideNx1_V_fp16(In, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Out, Filter, 0);
	if (PadR) KerConv2x1from3x1StrideNx1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 1);
}
...

{
	...
	if (PadT) KerConv1x2from1x3Stride1xN_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, 0);
	if (PadB) KerConv1x2from1x3Stride1xN_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 1);
}
...

{
	...
	if (PadL) KerConv2x3from3x3Stride1_V_fp16(In, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Out, Filter, 0);
	if (PadR) KerConv2x3from3x3Stride1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 1);
	if (PadT) KerConv3x2from3x3Stride1_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, 0);
	if (PadB) KerConv3x2from3x3Stride1_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 1);
}
...

{
	...
	if (PadL) KerConv2x3from3x3Stride2_V_fp16(In, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Out, Filter, 0);
	if (PadR) KerConv2x3from3x3Stride2_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 1);
	if (PadT) KerConv3x2from3x3Stride2_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, 0);
	if (PadB) KerConv3x2from3x3Stride2_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 1);
}
...

{
	...
	if (PadL) KerConv2x3from3x3StrideS_V_fp16(In, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Stride, Out, Filter, 0);
	if (PadR) KerConv2x3from3x3StrideS_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadTOrg, Wo, Ho, Ho_F, Ho_L, Stride, Out+Wo-1, Filter, 1);
	if (PadT) KerConv3x2from3x3StrideS_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Stride, Out+Wo_F, Filter, 0);
	if (PadB) KerConv3x2from3x3StrideS_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Stride, Out+Ho_L*Wo+Wo_F, Filter, 1);
}
...

{
	...
	if (PadL) {
		if (Wo_F==2) {
			KerConv4x1from5x1StrideNx1_V_fp16(In, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out, Filter, 2);
			KerConv4x1from5x1StrideNx1_V_fp16(In, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+1, Filter, 1);
		} else KerConv4x1from5x1StrideNx1_V_fp16(In, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out, Filter, PadL);
	}
	if (PadR) {
		if  ((Wo-Wo_L)==2) {
			KerConv4x1from5x1StrideNx1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-2, Filter, 3);
			KerConv4x1from5x1StrideNx1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 4);
		} else if (Wo==1) KerConv4x1from5x1StrideNx1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 5);
		else KerConv4x1from5x1StrideNx1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, PadR+2);
	}
}
...

{
	...
	if (PadT) {
		if(Ho_F==2) {
			KerConv1x4from1x5Stride1xN_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, 2);
			KerConv1x4from1x5Stride1xN_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F+Wo, Filter, 1);
		} else KerConv1x4from1x5Stride1xN_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, PadT);
	}
	if (PadB) {
		if((Ho-Ho_L)==2) {
			KerConv1x4from1x5Stride1xN_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 3);
			KerConv1x4from1x5Stride1xN_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+(Ho_L+1)*Wo+Wo_F, Filter, 4);
		} else if (Ho==1) KerConv1x4from1x5Stride1xN_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 5);
		else KerConv1x4from1x5Stride1xN_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, PadB+2);
	}
}
...

{
	...
	if (PadL==2) {
		KerConv4x5from5x5Stride1_V_fp16(In, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out, Filter, 2);
		KerConv4x5from5x5Stride1_V_fp16(In, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+1, Filter, 1);
	} else if (PadL==1) KerConv4x5from5x5Stride1_V_fp16(In, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out, Filter, 1);
	if (PadR==2) {
		if (Wo==1) KerConv4x5from5x5Stride1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 5);
		else {
			KerConv4x5from5x5Stride1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-2, Filter, 3);
			KerConv4x5from5x5Stride1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 4);
		}
	} else if (PadR==1) KerConv4x5from5x5Stride1_V_fp16(In+Wo_L*Stride-PadLOrg, W, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, 3);
	if (PadT==2) {
		KerConv5x4from5x5Stride1_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, 2);
		KerConv5x4from5x5Stride1_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F+Wo, Filter, 1);
	} else if (PadT==1) KerConv5x4from5x5Stride1_H_fp16(In, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, 1);
	if (PadB==2) {
		if (Ho==1) KerConv5x4from5x5Stride1_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+(Ho_L)*Wo+Wo_F, Filter, 5);
		else {
			KerConv5x4from5x5Stride1_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 3);
			KerConv5x4from5x5Stride1_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+(Ho_L+1)*Wo+Wo_F, Filter, 4);
		}
	} else if (PadB==1) KerConv5x4from5x5Stride1_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, PadLOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, 3);
}
...

{
	...
	if (PadL) KerConv4x5from5x5Stride2_V_fp16(In, W, H, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out, Filter, PadL);
	if (PadR) KerConv4x5from5x5Stride2_V_fp16(In+Wo_L*Stride-PadLOrg, W, H, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Out+Wo-1, Filter, PadR+2);

	if (PadT) KerConv5x4from5x5Stride2_H_fp16(In, W, H, PadLOrg, PadTOrg, Wo, Wo_F, Wo_L, Out+Wo_F, Filter, PadT);
	if (PadB) KerConv5x4from5x5Stride2_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, H, PadLOrg, PadTOrg, Wo, Wo_F, Wo_L, Out+Ho_L*Wo+Wo_F, Filter, PadB+2);
}
...

{
	...
	if (PadL) KerConv4x5from5x5StrideS_V_fp16(In, W, H, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Stride, Out, Filter, PadL);
	if (PadR) KerConv4x5from5x5StrideS_V_fp16(In+Wo_L*Stride-PadLOrg, W, H, PadOrg, Pad, Wo, Ho, Ho_F, Ho_L, Stride, Out+Wo-1, Filter, PadR+2);

	if (PadT) KerConv5x4from5x5StrideS_H_fp16(In, W, H, PadLOrg, PadTOrg, Wo, Wo_F, Wo_L, Stride, Out+Wo_F, Filter, PadT);
	if (PadB) KerConv5x4from5x5StrideS_H_fp16(In+(Ho_L*Stride-PadTOrg)*W, W, H, PadLOrg, PadTOrg, Wo, Wo_F, Wo_L, Stride, Out+Ho_L*Wo+Wo_F, Filter, PadB+2);
}
	...
	if (IterW&0x1) {
		for (unsigned int h=Ho_F; h<Ho_L; h++) {
			...
			for (unsigned int w=0; w<(IterW/2); w++) {
				LineOut[w] += ((F16V *)PtI)[w]*C0;
			}
			Out[Wo*h+Wo_L-1] += PtI[IterW-1]*c0;
		}
	} else {
		for (unsigned int h=Ho_F; h<Ho_L; h++) {
			F16V *LineOut = (F16V *) (&Out[Wo*h+Wo_F]);
...

{
	...
	if (IterW&0x1) {
		for (unsigned int h=Ho_F; h<Ho_L; h++) {
			...
			F16 *PtI = In + (h*Stride-PadT)*W + (Wo_F*Stride-PadL);
			for (unsigned int w=0; w<(IterW/2); w++) {
				LineOut[w] += gap_pack2f16(PtI[4*w]*C0, PtI[4*w+2]*C0);
			}
			Out[Wo*h+Wo_L-1] += PtI[2*(IterW-1)]*C0;
		}
	} else {
		for (unsigned int h=Ho_F; h<Ho_L; h++) {
			F16V *LineOut = (F16V *) (&Out[Wo*h+Wo_F]);
...

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) {
			...
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x1Stride1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
	       	}
	}
	gap_waitbarrier(0);
}

void KerParConv1x1Stride2_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) {
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x1Stride2_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
	       	}
	}
	gap_waitbarrier(0);
}

void KerParConv1x1StrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x1StrideS_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv3x1Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv3x1Stride1x1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv3x1BorderStrideNx1_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 1, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv3x1Stride2x1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv3x1Stride2x1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv3x1BorderStrideNx1_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 2, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv1x3Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x3Stride1x1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
				if ((int)PadIn) KerConv1x3BorderStride1xN_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 1, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv1x3Stride1x2_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x3Stride1x2_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv1x3BorderStride1xN_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 2, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv3x3Stride1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv3x3Stride1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv3x3BorderStride1_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv3x3Stride2_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv3x3Stride2_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv3x3BorderStride2_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv3x3StrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv3x3StrideS_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn);
			if ((int)PadIn) KerConv3x3BorderStrideS_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv5x1Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv5x1Stride1x1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv5x1BorderStrideNx1_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 1, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv5x1Stride2x1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv5x1Stride2x1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv5x1BorderStrideNx1_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 2, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv1x5Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x5Stride1x1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv1x5BorderStride1xN_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 1, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv1x5Stride1x2_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv1x5Stride1x2_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv1x5BorderStride1xN_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, 2, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv5x5Stride1_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv5x5Stride1_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv5x5BorderStride1_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv5x5Stride2_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv5x5Stride2_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn);
			if ((int)PadIn) KerConv5x5BorderStride2_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConv5x5StrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConv5x5StrideS_Body_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn);
			if ((int)PadIn) KerConv5x5BorderStrideS_fp16(in, out, filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConvNxNStrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FS*FS*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConvNxNStrideS_Body_fp16(in, out, filter, FS, FS, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn);
			if ((int)PadIn) KerConvNxNStrideS_Border_fp16(in, out, filter, FS, FS, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConvNxMStrideSxSy_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConvNxMStrideSxSy_Body_fp16(in, out, filter, FSx, FSy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadIn);
			if ((int)PadIn) KerConvNxMStrideSxSy_Border_fp16(in, out, filter, FSx, FSy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}

void KerParConvNxMDxDyStrideSxSy_fp16(KerConv_fp16_T *Arg)

{
	...
	unsigned int InFeatures = Arg->InFeatures;
       	...
       	for (unsigned int of=First; of<Last; of++) 
	       	for (unsigned int If=0; If<InFeatures; If++) {
			F16 *in = In+W*H*If, *filter = Filter+FSx*FSy*(TotalInFeatures*of  + If), *out = Out+Wo*Ho*(of);
			KerConvNxMDxDyStrideSxSy_Body_fp16(in, out, filter, FSx, FSy, Dx, Dy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadIn);
			if ((int)PadIn) KerConvNxMDxDyStrideSxSy_Border_fp16(in, out, filter, FSx, FSy, Dx, Dy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadIn, PadIn);
	       	}
	gap_waitbarrier(0);
}
...
/*
	...
	KerConv1x1Stride1_fp16
	KerConv1x1Stride2_fp16
	KerConv1x1StrideS_fp16

	KerConv3x1Stride1x1_fp16
		|------	KerConv3x1Stride1x1_Body_fp16
		|------	KerConv3x1StrideNx1_Border_fp16
	KerConv3x1Stride2x1_fp16
		|------	KerConv3x1Stride2x1_Body_fp16
		|------	KerConv3x1StrideNx1_Border_fp16
	KerConv1x3Stride1x1_fp16
		|------	KerConv1x3Stride1x1_Body_fp16
		|------	KerConv1x3Stride1xN_Border_fp16
	KerConv1x3Stride1x2_fp16
		|------	KerConv1x3Stride1x2_Body_fp16
		|------	KerConv1x3Stride1xN_Border_fp16
	KerConv3x3Stride1_fp16
		|------	KerConv3x3Stride1_Body_fp16
		|------	KerConv3x3Stride1_Border_fp16
	KerConv3x3Stride2_fp16
		|------	KerConv3x3Stride2_Body_fp16
		|------	KerConv3x3Stride2_Border_fp16
	KerConv3x3StrideS_fp16
		|------	KerConv3x3StrideS_Body_fp16
		|------	KerConv3x3StrideS_Border_fp16

	KerConv5x1Stride1x1_fp16
		|------	KerConv5x1Stride1x1_Body_fp16
		|------	KerConv5x1StrideNx1_Border_fp16
	KerConv5x1Stride2x1_fp16
		|------	KerConv5x1Stride2x1_Body_fp16
		|------	KerConv5x1StrideNx1_Border_fp16
	KerConv1x5Stride1x1_fp16
		|------	KerConv1x5Stride1x1_Body_fp16
		|------	KerConv1x5Stride1xN_Border_fp16
	KerConv1x5Stride1x2_fp16
		|------	KerConv1x5Stride1x2_Body_fp16
		|------	KerConv1x5Stride1xN_Border_fp16
	KerConv5x5Stride1_fp16
		|------	KerConv5x5Stride1_Body_fp16
		|------	KerConv5x5Stride1_Border_fp16
	KerConv5x5Stride2_fp16
		|------	KerConv5x5Stride2_Body_fp16
		|------	KerConv5x5Stride2_Border_fp16
	KerConv5x5StrideS_fp16
		|------	KerConv5x5StrideS_Body_fp16
		|------	KerConv5x5StrideS_Border_fp16

	KerConvNxNStrideS_fp16
		|------	KerConvNxNStrideS_Body_fp16
		|------	KerConvNxNStrideS_Border_fp16

	KerConvNxMStrideSxSy_fp16
		|------	KerConvNxMStrideSxSy_Body_fp16
		|------	KerConvNxNStrideSxSy_Border_fp16

	KerConvNxMDxDyStrideSxSy_fp16
		|------	KerConvNxMDxDyStrideSxSy_Body_fp16
		|------	KerConvNxNDxDyStrideSxSy_Border_fp16

*/

void KerConv1x1Stride1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) KerConv1x1Stride1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
	gap_waitbarrier(0);
}

void KerConv1x1Stride2_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) KerConv1x1Stride2_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
	gap_waitbarrier(0);
}

void KerConv1x1StrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) KerConv1x1StrideS_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadOrg);
	gap_waitbarrier(0);
}

void KerConv3x1Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv3x1Stride1x1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv3x1BorderStrideNx1_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv3x1Stride2x1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv3x1Stride2x1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv3x1BorderStrideNx1_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv1x3Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv1x3Stride1x1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv1x3BorderStride1xN_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sy, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv1x3Stride1x2_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv1x3Stride1x2_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv1x3BorderStride1xN_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sy, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv3x3Stride1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv3x3Stride1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv3x3BorderStride1_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv3x3Stride2_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv3x3Stride2_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv3x3BorderStride2_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv3x3StrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv3x3StrideS_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadOrg);
		if ((int)PadIn) KerConv3x3BorderStrideS_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}


void KerConv5x1Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv5x1Stride1x1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv5x1BorderStrideNx1_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv5x1Stride2x1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv5x1Stride2x1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv5x1BorderStrideNx1_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv1x5Stride1x1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv1x5Stride1x1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv1x5BorderStride1xN_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sy, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv1x5Stride1x2_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv1x5Stride1x2_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv1x5BorderStride1xN_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sy, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv5x5Stride1_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv5x5Stride1_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv5x5BorderStride1_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv5x5Stride2_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv5x5Stride2_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadOrg);
		if ((int)PadIn) KerConv5x5BorderStride2_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConv5x5StrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConv5x5StrideS_Body_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadOrg);
		if ((int)PadIn) KerConv5x5BorderStrideS_fp16(In, Out, Filter, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConvNxNStrideS_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConvNxNStrideS_Body_fp16(In, Out, Filter, FS, FS, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadOrg);
		if ((int)PadIn) KerConvNxNStrideS_Border_fp16(In, Out, Filter, FS, FS, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, S, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}


void KerConvNxMStrideSxSy_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConvNxMStrideSxSy_Body_fp16(In, Out, Filter, FSx, FSy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadOrg);
		if ((int)PadIn) KerConvNxMStrideSxSy_Border_fp16(In, Out, Filter, FSx, FSy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}

void KerConvNxMDxDyStrideSxSy_fp16(KerConv_fp16_T *Arg)

{
	...
	if (First<Last) {
		KerConvNxMDxDyStrideSxSy_Body_fp16(In, Out, Filter, FSx, FSy, Dx, Dy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadOrg);
		if ((int)PadIn) KerConvNxMDxDyStrideSxSy_Border_fp16(In, Out, Filter, FSx, FSy, Dx, Dy, W, H, Wo, Wo_F, Wo_L, Ho, Ho_F, Ho_L, Sx, Sy, PadIn, PadOrg);
	}
	gap_waitbarrier(0);
}
