...

{
    ...
    TILE_HOR,
		...
		KerArgs(2,
			KerArg("In",  KerArgSpace(1,KER_ITER_TILE0), OBJ_IN_DB,  Dim, Dim, sizeof(uint8_t),  0, 0, 0, "In"),
			KerArg("Out", KerArgSpace(1,KER_ITER_TILE0), OBJ_OUT_DB, Dim, Dim, sizeof(int32_t),  0, 0, 8, "Out")
		)
	...
	UserKernel(AppendNames(Name, "Vertical"),
		...
		KerArgs(1,
			KerArg("InOut", KerArgSpace(1,KER_ITER_TILE0), OBJ_IN_OUT_DB, Dim, Dim, sizeof(int32_t), 0, 0, 8, "InOut")
		)
...
void FFT2DConfiguration(unsigned int L1Memory, unsigned int L2Memory, unsigned int L3Memory)
{
  ...
  SetGeneratedFilesNames("FFT2DKernels.c", "FFT2DKernels.h");
