...

{
	...
	*/
	...
	for (int o=First; o<Last; o++) {
		...
		Oh = gap_clip(AT_SCALE((Infos[GRU_H_A0] - Oz)*StateInOut[o] + Oz*Oht, ((unsigned char *)Infos)[GRU_H_SCALE], ((unsigned char *)Infos)[GRU_H_SCALEN]), 7);

		if (StateInOut) StateInOut[o] = Oh;
		if (Hout) Hout[o] = Oh;
