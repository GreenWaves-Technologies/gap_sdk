...
# Final binary
#------------------------------------------
GWT_PMSIS	= $(GAP_SDK_HOME)/PMSIS
PMSIS_BACKEND	= $(GAP_SDK_HOME)/mbed-os/targets/TARGET_GWT/pmsis_backend
...
C_OBJECTS     = $(patsubst %.c, $(BUILDDIR)/%.o, $(wildcard $(shell find $(MBED_PATH)/mbed-os -name "*.c" \
		...
		-not -path "$(MBED_PATH)/mbed-os/targets/TARGET_GWT/TARGET_*"\
		-not -path "$(MBED_PATH)/mbed-os/targets/TARGET_GWT/pmsis_backend/*")))

C_OBJECTS     += $(patsubst %.c, $(BUILDDIR)/%.o, $(wildcard $(shell find $(MBED_PATH)/mbed-os/targets/TARGET_GWT/TARGET_$(chip) -name "*.c")))

C_OBJECTS     += $(patsubst %.c, $(BUILDDIR)/%.o, $(shell find $(GWT_PMSIS) -iname "*.c"))
C_OBJECTS     += $(patsubst %.c, $(BUILDDIR)/%.o, $(shell find $(PMSIS_BACKEND) -iname "*.c"))
...
INC_PATH      += $(foreach d, $(INC), -I$d)  $(INC_DEFINE) $(foreach f, $(shell find $(GWT_PMSIS) $(PMSIS_BACKEND) -iname "*.h" -exec dirname {} \;), -I$f)
...
ifeq ($(chip), GAP8)
	...
	$(MBED_PATH)/tools/runner/run_gap9.sh $(PLPBRIDGE_FLAGS) -ftdi
endif
