...
def parse_rules_str(contents, base_dir):
    rules = []
    counter = 0
...
def parse_rules(full_path, base_dir=None):
    if base_dir is None:
...
def parse_gitignore(full_path, base_dir=None):
    return gen_match(parse_rules(full_path, base_dir=base_dir))
