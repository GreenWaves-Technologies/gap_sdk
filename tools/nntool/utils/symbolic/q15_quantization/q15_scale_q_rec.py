...
class Q15ScaleQRec(QRecBase):
    def __init__(self, dtype: np.dtype, scale: float, q: int) -> None:
        self._dtype = dtype
        self._scale = scale
        self._q = q
    ...
    @classmethod
    def inherit(cls, rec, dtype: np.dtype = None, scale: float = None, q: int = None):
        if rec is None:
            return cls(dtype, scale, q)
        if rec.q is None:
            rec.q = q
    ...
    def quantize_c_expr(self, val):
        return "((%s)round(2^(%s) * (%s)/(%s), 0))" % (self.DTYPE_TO_CTYPE[self.dtype], self.q, val, self.scale)

    def quantize_and_clip_c_expr(self, val):
        iinfo = np.iinfo(self.dtype)
        bits = math.ceil(math.log2(iinfo.max))
        return "gap_clip(((%s)round(2^(%s) * (%s)/(%s), 0)), %s)" % (self.DTYPE_TO_CTYPE[self.dtype], self.q, val, self.scale, bits)
