...
from utils.symbolic.quantization_base import QuantizationHandlerBase
from utils.symbolic.symbol import Symbol, Variable
...
class FunctionCollection():
    ...
    def produce_functions(self, produced_idx, execution_order, index_dependencies, depth, code_block):
        while (produced_idx < len(execution_order) and
            ...
            code_block.write("{} = {};", var.c_expr(declare=declare, dtype=np.int32),
                             self._functions[var].c_expr())
            produced_idx += 1
        return produced_idx

    def quantize(self, quantizer: QuantizationHandlerBase, symbol_control, quantize_inputs=False):
        funcs = []
        out_qrecs = {}
        in_qrecs = {}
        for var, func in self._functions.items():
            ...
            if var.name in self._outputs:
                qfunc, qrec = quantizer.quantize_output(
                    ...
                    quantize_inputs=quantize_inputs,
                    prequantized_variables=out_qrecs)
            #     var = Variable(var.name, shape=var.shape, dtype=np.int8)
            # else:
            #     var = Variable(var.name, shape=var.shape, dtype=np.int32)
