...
from ..matcher import Matcher, description, groups, match_name, run_after, modifies_dimensions
        ...
        for node in tnodes:
            if node.transpose_in:
                for idx, edge in enumerate(G.in_edges(node.name)):
                    ...
                    if G.quantization:
                        G.quantization.copy_qrec(
                            node, 'in', edge.to_idx, in_params)
            ...
            if node.transpose_out:
                for idx, edge in enumerate(G.out_edges(node.name)):
                    ...
                    if G.quantization:
                        G.quantization.copy_qrec(
                            node, 'out', edge.from_idx, out_params)
