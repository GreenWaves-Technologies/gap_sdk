...
from graph.matches.find_sequence import (AnyOf, NodeMatcher, OneOrMoreOf,
                                         Sequence)
from graph.types import (ActivationParameters, ConstantInputParameters,
                         MatrixBroadcastedLinearOpParameters, NNEdge,
                         ReshapeParameters)
...
def propagate_shape(G, shape, in_pattern, nodes):
    for node in nodes:
        if isinstance(node, MatrixBroadcastedLinearOpParameters):
...
class SqueezeExpandMatcher(Matcher):
    ...
    def _match(self, G: GraphView, set_identity: bool = True, **kwargs):
        ...
        if set_identity:
            self.set_identity(G)
        seq = Sequence(ReshapeParameters, OneOrMoreOf(
            AnyOf(ActivationParameters, CustomMatcher())), AnyOf(ReshapeParameters, FcParameters))
        for nodes in seq.find(G):
            in_pattern = nodes[0].exp_red_pattern()
            if not in_pattern:
                continue
            has_modified_graph = True
            remove_both = isinstance(nodes[-1], ReshapeParameters) and nodes[0].old_shape.shape == nodes[-1].shape.shape
