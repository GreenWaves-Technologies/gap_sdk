...
from graph.types import (ActivationParameters, MatrixAddParameters,
                         MatrixMulParameters, ConstantInputParameters, NNEdge)
    ...
    def add_node(self, params, G):
        if isinstance(params, MatMulOpParameters):
            ...
            for in_edge in G.in_edges(params.name):
                can_fuse = can_fuse or isinstance(in_edge.from_node, ConstantInputParameters)
            if not can_fuse:
                return None
            self.order.append(params)
            self.add = params
    ...
    def match(self, G: GraphView, set_identity: bool = True):
        ...
        for matmul_node in [params for params in G.nodes() if isinstance(params, MatMulOpParameters)]:
            ...
            if node_list.active is not None:
                subgraph.add_edge(NNEdge(from_node=node_list.matmul, to_node=node_list.active))
            input_mapping = [[(node_list.matmul, idx)] for idx in range(2)]
            if node_list.add:
                input_mapping += [[(node_list.matmul, 2)]]
            output_mapping = [(node_list.active, 0)] if node_list.active else [(node_list.matmul, 0)]
            ...
            if node_list.add:
                bias_edge = [add_edge for add_edge in G.in_edges(node_list.add.name) if isinstance(add_edge.from_node, ConstantInputParameters)][0]
            ...
            if node_list.add:
                G.add_edge(NNEdge(bias_edge.from_node, pnode, from_idx=bias_edge.from_idx, to_idx=2))
