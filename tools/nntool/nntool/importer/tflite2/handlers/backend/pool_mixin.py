...
from nntool.importer.tflite2.tflite_schema_head.Pool2DOptions import Pool2DOptions
...
class PoolMixin(FilterPadMixin):
    ...
    def pool2d(cls, node, pool_type=None, **kwargs):
        ...
        if filter_matches_input and stride_is_one:
            params = GlobalPoolingNodeBase(node.name,
                                          pool_type=pool_type,
                                          axis=[0, 1],
                                          keep_dims=True,
                                          in_dims_hint=[['h', 'w', 'c']],
            ...
            params = PoolingNodeBase(node.name,
                                       filter_dim=PoolFilterDim(filt_h, filt_w),
