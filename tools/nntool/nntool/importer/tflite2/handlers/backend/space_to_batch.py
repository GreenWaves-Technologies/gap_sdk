...
from nntool.utils.batch_to_space import (space_to_batch_nd,
                                         space_to_batch_shape_nd)

from ..backend_handler import BackendHandler
from ..handler import tflite_op


@tflite_op("SPACE_TO_BATCH_ND")
class SpaceToBatch(BackendHandler, ConstantMixin):
    ...
    def _common(cls, node: TFLiteNode, **kwargs):
        ...
        pads = list(list(elem) for elem in cls._verify_constant(inputs[2]).astype(np.int32))

        if any(dim is None for dim in x_shape[1:]):
            raise TFLiteImportException(
                f'SPACE_TO_BATCH_ND {node.name} only batch dim can be unknown')

        if cls._is_constant(x):
            LOG.info("reducing %s to a constant", node.name)
            value = space_to_batch_nd(cls._get_constant(x), blocks, pads)
            ...
            params = SpaceToBatchNode(node.name, block_shape=blocks, pads=pads)
            out_shape = space_to_batch_shape_nd(x_shape, blocks, pads)
