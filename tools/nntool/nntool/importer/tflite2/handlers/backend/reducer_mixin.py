...
from nntool.graph.types import GlobalPoolingNodeBase, NNEdge, NoOPNode
from nntool.importer.common.provisional_dim import ProvisionalDim
from nntool.importer.tflite2.tflite_schema_head.ReducerOptions import ReducerOptions
...
class ReducerMixin(object):
    ...
    def _common(cls, node, **kwargs):
        ...
        if not BackendHandler.remove_unspecified_dim(axes):
            ...
            params = GlobalPoolingNodeBase(node.name, pool_type=reduce_type, axis=tuple(axes),
                                          keep_dims=node_opts.KeepDims())
            # the reduced axes are set to 1 in the output shape

        if opts.get('load_quantization'):
            G.quantization[params.name] = BackendHandler.load_tf_quantization(
