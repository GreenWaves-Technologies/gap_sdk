...
from nntool.graph.types import (ConstantInputNode, NNEdge, NoOPNode,
                         ReshapeNode, StridedSliceNode)
...
from nntool.importer.tflite2.tflite_schema_head.StridedSliceOptions import \
    StridedSliceOptions
...
class StridedSlice(ConstantMixin, BackendHandler):
    ...
    def _common(cls, node: TFLiteNode, **kwargs):
        ...
        if cls.is_constant(x):
            ...
            params = StridedSliceNode(node.name, act_slice=act_slice, out_shape=out_shape)
            ...
            if can_reshape:
                if list(x_shape) == list(out_shape):
                    ...
                    params = ReshapeNode(node.name, old_shape=in_shape, shape=out_shape)
                G.add_edge(NNEdge(from_node=x[0], to_node=params, from_idx=x[1], to_idx=0))
            else:
                params = StridedSliceNode(node.name, act_slice=act_slice, out_shape=out_slice_shape)
