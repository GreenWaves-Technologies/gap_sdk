...
from nntool.graph.types import NNEdge, RNNNode
from nntool.importer.common.provisional_dim import ProvisionalDim
from nntool.importer.tflite2.tflite_schema_head.SequenceRNNOptions import \
    SequenceRNNOptions
    ...
    def _common(cls, node, **kwargs):
        ...
        constant_nodes = cls.get_all_const_inputs(
            ...
            names=["%s_%s" % (in_name, node.name)
                   for in_name in RNNNode.INPUT_NAMES],
            short_names=RNNNode.INPUT_NAMES,
        ...
        for state_node_name in ['i_state']:
            state_node = constant_nodes[RNNNode.INPUT_NAMES.index(state_node_name)]
