...
class Fill(BackendHandler):
    ...
    def _common(cls, node: TFLiteNode, **kwargs):
        ...
        x_shape = x[2].shape
        if len(x_shape) != 1:
            raise TFLiteImportException(f'FILL {node.name} expecting 1D tensor for shape')
        ...
        if cls._is_constant(inputs[1]):
            ...
            raise TFLiteImportException(f'FILL {node.name} non constant fill values are not currently supported')
