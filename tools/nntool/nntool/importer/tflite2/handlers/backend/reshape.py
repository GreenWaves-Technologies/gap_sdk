...
from nntool.importer.tflite2.tflite_schema_head.ReshapeOptions import ReshapeOptions
...
class Reshape(BackendHandler, ConstantMixin):
    ...
    def _common(cls, node: TFLiteNode, **kwargs):
        ...
        new_shape = None
        if node_opts and len(inputs) == 1:
            new_shape = list(node_opts.NewShapeAsNumpy().flatten())
        elif len(inputs) > 1:
            new_shape = list(cls._verify_constant(inputs[1]).flatten())
            node.input[1].used = True
        else:
            TFLiteImportException(
        ...
        if cls.is_constant(x):
            ...
            params = ConstantInputNode(node.name, value=val,
