...
from nntool.graph.matches.matchers.remove_quantize_operators import \
    RemoveQuantizeOperators
from nntool.graph.matches.matchers.remove_reshapes_before_linear import \
...
from nntool.importer.tflite2.common.tflite_graph import TFLiteGraph
from nntool.importer.tflite2.tflite_schema_head.Model import Model
...
from nntool.utils.add_sys_path import add_sys_path
...
class TFLiteImporter(ImporterBase):
    ...
    def find_nodes_with_bad_quantization(G):
        ...
        for edge in G.edges():
            ...
            eqrecs = tuple(qrecs.get(node.name) for node in nodes)
    ...
    def create_graph(self, filename, opts):
        ...
        if opts.get('load_quantization'):
            ...
            G.quantization.schemes_present.add('scaled')
        variables = {}
        self._import_tflite_graph(G, model, opts, variables)
    ...
    def _import_tflite_graph(self, G: NNGraph, model, opts: dict, variables, subgraph_idx=0):
        name_cache = set()
        graph = TFLiteGraph.from_model(
            model, subgraph_idx, anonymise=opts.get('anonymise'), name_cache=name_cache)
        ...
        self._import_nodes(G, graph, handlers, all_nodes,
                           self._provisional_outputs, opts, variables=variables)
        return G

    def import_tflite_subgraph(self, G: NNGraph, model, subgraph_idx, variables, opts: dict):
        return self._import_tflite_graph(G, model, opts, variables, subgraph_idx=subgraph_idx)
    ...
    def _load_quantization(qrecs, node_recs):
        for tensor in node_recs:
            ...
            if qtype:
                ...
                qrecs[node_recs[tensor][0].name] = QRec.scaled(
    ...
    def _import_nodes(self, G, graph, handlers, all_nodes, outputs, opts, **kwargs):
        for node in graph.nodes:
            ...
            params = handler.handle(
                node, all_nodes=all_nodes, G=G, opts=opts, importer=self, outputs=outputs, **kwargs)
            if params is None:
                continue
            for idx, out_tensor in enumerate(node.output):
                ...
                if opts.get('load_quantization'):
                    qtype = deepcopy(
                        G.quantization[params.name].out_qs[idx])
                    G.quantization[output[0].name] = QRec.scaled(
                        in_qs=[qtype],
                        out_qs=[qtype]
                    )
