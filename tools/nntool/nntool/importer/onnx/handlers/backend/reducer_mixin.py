...
from nntool.graph.types import (ConstantInputNode, GlobalPoolingNodeBase,
                         NNEdge, NoOPNode)
    ...
    def _common(cls, node, copy_qtype=False, constant_operation=None, **kwargs):
        ...
        if not stripped_axes:
            ...
            if cls.is_constant(x) and constant_operation:
                ...
                params = ConstantInputNode(valid_name, value=val, dims=Dim.unnamed(val.shape))
            else:
                params = GlobalPoolingNodeBase(valid_name, pool_type=reduce_type, axis=stripped_axes,
                                                 keep_dims=keep_dims)
