...
import numpy as np
from nntool.graph.types import ConstantInputNode, ExpandNode
...
class Expand(BroadcastMixin, ConstantMixin, BackendHandler):
    ...
    def _common(cls, node, **kwargs):
        ...
        if cls.is_constant(x):
            ...
            params = ConstantInputNode(valid_name, value=x_val * np.ones(shape))
        else:
            params = ExpandNode(valid_name, shape=shape)
