...
from nntool.graph.types import (ConstantInputNode, LogOpNode, NNEdge,
                         SoftMaxNode)
...
class LogSoftmax(ConstantMixin, BackendHandler):
    ...
    def _common(cls, node, v13=False, **kwargs):
        ...
        if cls.is_constant(x):
            ...
            params = ConstantInputNode(valid_name, value=np.log(softmax_func(cls.get_constant(x), axis=axis)))
        else:
            softmax_params = SoftMaxNode(f'{valid_name}_softmax', axis=axis)
            G.add_edge(NNEdge(from_node=x[0], to_node=softmax_params, from_idx=x[1], to_idx=0))
            params = LogOpNode(f'{valid_name}_log')
