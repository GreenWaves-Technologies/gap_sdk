...
class GRU(RNNMixin, ConstantMixin, BackendHandler):
    ...
    def _common(cls, node, **kwargs):
        ...
        if i_weights.shape != (num_directions, hidden_size * 3, input_size):
            raise ValueError(f'{valid_name} input weights have bad shape {i_weights.shape}')
        r_weights = cls.get_constant(inputs[2])
        if r_weights.shape != (num_directions, hidden_size * 3, hidden_size):
            raise ValueError(f'{valid_name} state weights have bad shape {r_weights.shape}')
        biases = cls.get_constant(inputs[3]) if inputs[3] else np.zeros(
            [num_directions, 6 * hidden_size])
        if biases.shape != (num_directions, 6 * hidden_size):
            raise ValueError(f'{valid_name} biases have bad shape {biases.shape}')
        ...
        return cls.attach_rnn(G, x, GRUNode, extra_args, valid_name, tensors,
