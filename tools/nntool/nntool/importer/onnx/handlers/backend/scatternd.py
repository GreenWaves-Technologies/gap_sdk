...
from nntool.graph.types.tensor_shape import ScatterNdNode
...
def scatter_indices_regular(x_shape, indices):
    if indices.shape[-1] != len(x_shape):
        return False
    if len(indices.shape) -1 != len(x_shape):
        return False
    index = [0] * len(x_shape)
    while True:
        ...
        while True:
            index[idx] += 1
            if index[idx] >= indices.shape[idx]:
                if idx == 0:
                    return True
                index[idx] = 0
                idx -= 1
...
class ScatterND(ConstantMixin, BackendHandler):
    ...
    def _common(cls, node, **kwargs):
        ...
        if cls.is_constant(x) and cls.is_constant(updates):
            ...
            params = ConstantInputNode(valid_name, value=scatter_nd_impl(x_val, indices, updates_val, reduction=reduction))
        else:
            reg = scatter_indices_regular(x_shape, indices)
            logger.warning(f'{valid_name} ScatterND is not currently supported in the nntool or Autotiler kernels')
            params = ScatterNdNode(valid_name, indices=indices, updates=updates, reduction=reduction)
