...
from nntool.utils.stats_funcs import qsnr, cos_similarity

from nntool.execution.graph_executer import GraphExecuter
from nntool.execution.quantization_mode import QuantizationMode

from nntool.stats.stats_collector import ReductionStatsCollector
        ...
        for idx, fstat in enumerate(foutputs):
            ...
            if fstat['fusion_outputs']:
                for jdx, ffstat in enumerate(fstat['fusion_outputs']):
                    nid = (fstat['node'].name, ffstat['node'].name)
                    stats[nid] =\
