...
from nntool.graph.types.base import FilterNodeBase
from nntool.utils.tabular import Tabular, TabularColumn
...
class QuantizationReporter():
    ...
    def report(self, G, stats, nodes=None):
        ...
        for key, qrec in stats.sorted_iterator(G):
            ...
            node = fnode or node
            if qrec:
                if (not single or not qrec.ktype == 'scaled' or not isinstance(node, FilterNodeBase) or
