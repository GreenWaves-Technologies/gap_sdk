...
from nntool.utils.fft_quant import (Rad2_FFT_DIF_Fix16, Rad4_FFT_DIF_Fix16,
                             RFFT_Step_Fix16, SwapSamples)
from nntool.utils.numpy_helpers import np_asscalar
from nntool.utils.pow_sqrt import (LN_2_1F15, LN_10_INV_Q10, LOG10_2, gap_fl1,
...
class DSPKernelBasePow2(KernelBase):
    @classmethod
    def preemphasis(cls, params: DSPNodeBase, in_data, qfft_in):
    ...
    @classmethod
    def fft_step(cls, params: DSPNodeBase, in_cfft, fft_twiddles, swap_table):
        if params.is_radix4():
    ...
    @classmethod
    def spectrogram_step(cls, params: DSPNodeBase, in_data, shift, fft_out_q):
        spectrogram = (np.uint32(in_data.real) ** 2) + \
            (np.uint32(in_data.imag) ** 2)
    ...
    @classmethod
    def melspectrogram_step(cls, params: DSPNodeBase, in_data, filterbanks_sparsity, filterbank_coeff, mel_coeff_q):
        melspectrogram = []
        shift_buff = []
        for i, (start, nonzero_items, base) in enumerate(filterbanks_sparsity):
            ...
            for j in range(start, start+nonzero_items):
                melbin += filterbank_coeff[base +
                                           j-start] * (in_data[j] >> shift)
    ...
    @classmethod
    def norm_clip_32_melspect(cls, params: DSPNodeBase, in_data, shift_buff):
        norm = (30 - shift_buff) - 16
        return np.where(norm < 32, in_data >> norm, 0).astype(np.int32)

    @classmethod
    def log_step(cls, params: DSPNodeBase, in_data, mel_coeff_q, shift_buff, fft_out_q, shift, norm):
        ...
        if params.log_type == "db":
            return np.clip(
                at_norm(
                    10 * ((logn_17_15(in_data, True) * LN_10_INV_Q10 >>
                           10) - (qformat - 15) * LOG10_2),
                    norm), -(1 << 15), (1 << 15)-1).astype(np.int16)
        return np.clip(at_norm(logn_17_15(in_data, True) - (qformat - 15) * LN_2_1F15, norm),
                       -(1 << 15),
...
class Rfft2DSymmetric(DSPKernelBasePow2):
    @classmethod
    def execute(cls, params: DSPNodeBase,
                in_tensors,
                qrec: QRec,
                **kwargs):
        ...
        for frame_idx in range(params.n_frames):
            in_data = in_tensors[0][params.frame_step *
                                    frame_idx:params.frame_step*frame_idx+params.frame_size]
            in_data, shift = cls.preemphasis(
                params, in_data, 12 if params.is_radix4() else 13)
            if params.win_fn:
                win_lut = in_tensors[1]
                in_data = cls.windowing(
                    params, in_data, win_lut, qrec.in_qs[1].q)
            ...
            spectrograms.append(cls.spectrogram_step(
                params, out_data, shift, qrec.cache['fft_out_q'].q))
...
class Mfcc2DSymmetric(DSPKernelBasePow2):
    ...
    def execute(cls, params: DSPNodeBase,
        ...
        for frame_idx in range(params.n_frames):
            ...
            out_data = out_data[0] + 1j*out_data[1]
            spectrogram = cls.spectrogram_step(
                params, out_data, shift, qrec.cache['fft_out_q'].q)

            melspect, shift_buff = cls.melspectrogram_step(
                params, spectrogram, mel_filterbank_sparsity_mat, mel_filterbank_coeff, qrec.in_qs[6].q)
            if params.mel_type == "melspectrogram":
                result.append(cls.norm_clip_32_melspect(
                    params, melspect, shift_buff))
                continue

            logmelspect = cls.log_step(
                params, melspect, qrec.in_qs[6].q, shift_buff, qrec.cache["fft_out_q"], shift, params.quant_norm)
            ...
            if params.n_dct:
                mfcc = np.clip(
                    at_norm(np.dot(dct_matrix, logmelspect), 14), -(1 << 15), (1 << 15)-1)
                result.append(mfcc)
