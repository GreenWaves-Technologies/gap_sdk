    ...
    def execute(cls, params,
        ...
        for idx in range(params.n_cells):
            if isinstance(params, LSTMNode):
                ...
                res, new_i_state = cls.step_kernel(params, args, idx, in_tensor, qrec)
...
class RNNSymmetric(RnnSymmetricMixin, KernelBase):
    ...
    @classmethod
    def step_kernelu8_u8(cls, params: GRUNode,
                         args: Mapping[str, np.ndarray],
                         idx: int,
                         input_tensor: np.ndarray,
                         qrec):
        ...
        return output_gate_scratch, args['i_state'][0]

    @classmethod
    def step_kernelu16_u8(cls, params: GRUNode,
                          args: Mapping[str, np.ndarray],
                          idx: int,
                          input_tensor: np.ndarray,
                          qrec):
    ...
    @classmethod
    def step_kernel8_8(cls, params: GRUNode,
                       args: Mapping[str, np.ndarray],
                       idx: int,
                       input_tensor: np.ndarray,
                       qrec):
    ...
    @classmethod
    def step_kernel16_8(cls, params: GRUNode,
                        args: Mapping[str, np.ndarray],
                        idx: int,
                        input_tensor: np.ndarray,
                        qrec):
...
class GRUSymmetric(RnnSymmetricMixin, KernelBase):
    ...
    def step_kernelu16_u8(cls, params: GRUNode,
        ...
        DiagCollector.record('h_state_out_prenorm', h_state,
                             scale=math.pow(2, -30),
                             node=params)
        h_state = at_norm(h_state, 30-qrec.out_qs[0].q) # normalize to output q (normally Q15)
        h_state = qrec.out_qs[0].clip(h_state + qrec.out_qs[0].zero_point)
...
def scale_lstm_cellout(qrec, tensor: np.ndarray, axis: int):
    external_type = qrec.in_qs[LSTMNode.INPUT_NAMES.index('c_state')]
...
class LSTMSymmetric(RnnSymmetricMixin, KernelBase):
    ...
    # NE16 8 bit kernel
    @ classmethod
    def step_kernelu8_u8(cls, params: LSTMNode,
                         args: Mapping[str, np.ndarray],
                         idx: int,
                         input_tensor: np.ndarray,
                         qrec):
        ...
        return output, args['i_state'][0], args['c_state'][0]
    ...
    @ classmethod
    def step_kernelu16_u8(cls, params: LSTMNode,
                         args: Mapping[str, np.ndarray],
                         idx: int,
                         input_tensor: np.ndarray,
                         qrec):
    ...
    @ classmethod
    def step_kernel8_8(cls, params: LSTMNode,
                       args: Mapping[str, np.ndarray],
                       idx: int,
                       input_tensor: np.ndarray,
                       qrec):
    ...
    @classmethod
    def step_kernel16_8(cls, params: LSTMNode,
                        args: Mapping[str, np.ndarray],
                        idx: int,
                        input_tensor: np.ndarray,
                        qrec):
