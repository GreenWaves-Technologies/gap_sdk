...
import numpy as np
from nntool.graph.types import LSTMNode, RNNNode
...
from nntool.utils.diag_collector import DiagCollector

LOG = logging.getLogger(__name__)


def abs_clip(arr: np.ndarray, abs_limit):
    return np.clip(arr, -abs_limit, abs_limit).astype(arr.dtype)
    ...
    def execute(cls, params,
        ...
        if details is not None:
            init_stats(
                details,
                'range_state')
            if isinstance(params, LSTMNode):
                DiagCollector.active_set('__rnn_quant')
                init_stats(
                    details,
                    'range_cell',
                )
            elif isinstance(params, (GRUNode, RNNNode)):
                DiagCollector.active_set('__rnn_quant')

        for idx in range(params.n_cells):
            if isinstance(params, LSTMNode):
                res, new_i_state, new_c_state = cls.step_kernel(
                    params, args, idx, in_tensor, details=details)
            else:
                res, new_i_state = cls.step_kernel(
                    params, args, idx, in_tensor, details=details)
        ...
        if params.output_directions:
            ...
            new_i_state = np.expand_dims(new_i_state, 0)
            if isinstance(params, LSTMNode):
                new_c_state = np.expand_dims(new_c_state, 0)
        out_params = [out_tensor]
        if params.output_h_state:
            out_params.append(new_i_state)
        if isinstance(params, LSTMNode) and params.output_c_state:
            out_params.append(new_c_state)
        return out_params
...
class RNNFloat32(RnnFloat32Mixin, KernelBase):
    @classmethod
    def step_kernel(cls, params: RNNNode,
        ...
        return input_gate_scratch, args['i_state']
...
class GRUFloat32(RnnFloat32Mixin, KernelBase):
    @classmethod
    def step_kernel(cls, params: GRUNode,
                    args: Mapping[str, np.ndarray],
                    idx: int,
                    input_tensor: np.ndarray,
                    **kwargs):
        ...
        return args['h_state'].copy(), args['h_state'].copy()
...
class LSTMFloat32(RnnFloat32Mixin, KernelBase):
    @classmethod
    def step_kernel(cls, params: LSTMNode,
                    args: Mapping[str, np.ndarray],
                    idx: int,
                    input_tensor: np.ndarray,
                    **kwargs):
        ...
        if idx < params.n_input_cells:
            if not use_cifg:
                input_gate_scratch += args['i_2_i_w'].dot(input_tensor[idx]).astype(input_gate_scratch.dtype)
            # in = ScaleW * ScaleI
            forget_gate_scratch += args['i_2_f_w'].dot(input_tensor[idx]).astype(forget_gate_scratch.dtype)
            cell_scratch += args['i_2_c_w'].dot(input_tensor[idx]).astype(cell_scratch.dtype)
            output_gate_scratch += args['i_2_o_w'].dot(input_tensor[idx]).astype(output_gate_scratch.dtype)
        ...
        # For each cell: compute recurrent_weight * output_state
        if not use_cifg:
            input_gate_scratch_state = args['r_2_i_w'].dot(args['i_state']).astype(args['r_2_i_w'].dtype)
            DiagCollector.record('i_gate_r', input_gate_scratch_state, node=params)
        # in + ScaleRW * ScaleOState
        forget_gate_scratch_state = args['r_2_f_w'].dot(args['i_state']).astype(args['r_2_f_w'].dtype)
        cell_scratch_state = args['r_2_c_w'].dot(args['i_state']).astype(args['r_2_c_w'].dtype)
        output_gate_scratch_state = args['r_2_o_w'].dot(args['i_state']).astype(args['r_2_o_w'].dtype)
        ...
        return output_gate_scratch, args['i_state'], args['c_state']
