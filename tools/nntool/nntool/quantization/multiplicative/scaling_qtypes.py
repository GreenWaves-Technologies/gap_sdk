...
import numpy as np
from nntool.quantization.multiplicative.utils.scale import (apply_scales,
                                                     compute_scales)
...
class MultMulBiasScaleQType(ScalingQType):
    ...
    def __init__(self, *args, scale=None, dtype=np.uint8, available_bits=None, calc_dtype=np.int32, **kwargs):
        ...
        self._calc_dtype = calc_dtype
        self.scale = scale

    @classmethod
    def from_filter(cls, in_q, weights_q, out_q, params, initial_bits_accs=0, dtype=np.uint8):
        available_bits = (
            31 - (math.ceil(math.log2(params.filter_dim.sz)) + 7 + 7))
