...
from nntool.graph.types import (ConstantInputNode, ExpressionFusionNode,
                         FusionNodeBase, FusionInputNode,
...
def build_stat_from_qtypes(in_qs, out_qs, node=None):
    ...
    if not range_in_valid or not range_out_valid:
        ...
        if isinstance(node, QuantizeNode) and (range_in_valid or range_out_valid):
            if range_in_valid:
...
def build_fusion_stats(stats: dict, fusion: FusionNodeBase):
    ...
    for sub_node in outputs:
        ...
        stat = stats.get(edge.from_node.name)
        if stat is None:
            out_stats = None
            break
...
def set_stats(G, current_stats=None, current_graph_options=None, current_node_options=None):
    stats = {} if current_stats is None else current_stats.copy()
    current_node_options = {} if current_node_options is None else current_node_options.copy()
    current_graph_options = {} if current_graph_options is None else current_graph_options.copy()
    for node in G.nodes():
        nid = node.name
        ...
        if isinstance(node, FusionNodeBase) and node.quantize_internals:
            for fnode in node.subgraph.nodes():
                ...
                if nid not in stats:
                    ...
                    if isinstance(fnode, (FusionInputNode, FusionOutputNode)):
                        continue
                    else:
            ...
            if G.quantization and nid in G.quantization:
                current_node_options.setdefault(nid, {})['qtype_ind'] = G.quantization[nid].out_qs[0]

    return stats, current_graph_options, current_node_options
