...
from nntool.utils.event_listener import EventEmitter
from nntool.utils.json_serializable import JsonSerializable
from nntool.utils.stats_funcs import calc_bits

from nntool.quantization.multiplicative.mult_utils import compute_mul_bias
...
class QType(JsonSerializable, EventEmitter):
    ...
    def calculate_scale(rmin, rmax, qmin, qmax, dtype, asymmetric=False,
        ...
        if zero_point is not None:
            ...
            # NEW VERSION
            # nudged_zero_point = np.ceil(zero_point_from_min).astype(dtype)
    ...
    def dump(self, tensors, key: Union[Tuple, str], code_block: StreamCodeBlock, indent=True, nl='\n'):
        if self.is_floating:
            code_block.write(f'QType(dtype={repr(self.dtype)})', indent=indent)
            return
        if self.is_pow2:
            code_block.write(f'QType(bits={self.bits}, q={self.q}, dtype={repr(self.dtype)})', indent=indent)
            return
        code_block.write('QType(', indent=indent).indent()
        if np.any(self.zero_point != 0):
            code_block.write(f'zero_point={repr(self.zero_point)},')
        code_block.write(f'bits={self.bits},')
        for attr in ['scale', 'min_val', 'max_val']:
            ...
            if len(val) > 1:
                ...
                code_block.write(f'{attr}=tensors[{repr(this_key)}],')
            else:
                code_block.write(f'{attr}={repr(val.item())},')
        if self.quantized_dimension is not None:
            code_block.write(f'quantized_dimension={repr(self.quantized_dimension)},')
        code_block.write(f'dtype={self.dtype.__name__})', nl=nl).deindent()
    ...
    def __repr__(self) -> str:
        ...
        return 'QType('+", ".join(f'{k}={repr(getattr(self, k))}' for k in self.EXPORT if getattr(self, k) is not None)+')'
