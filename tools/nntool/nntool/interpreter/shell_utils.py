...
from nntool.utils.data_importer import MODES
from nntool.utils.tabular import CSVRenderer, ExcelRenderer, TextTableRenderer
...
def input_options(parser):
    ...
    parser.add_argument('-T', '--transpose',
                        action="store_true",
                        help='Used on images (it swap the channels position (from CHW to HWC or viceversa)) or \
                                    on 2D arrays swapping the 2 dimesnions')
    ...
    parser.add_argument('--zero_pad',
                        type=int, nargs="*", default=None,
                        help="amount of zero pad samples added before and after each axis: \
                            np.pad((before_0, after_0), (before_1, after_1), ..), i.e. --zero_pad 0 24, \
                            Every axis needs to be explicit. The axis order is the one after the optional transpose")
    parser.add_argument('--slices',
                        type=int, nargs="*", default=None,
                        help="Slices of each axis in order: start_0, stop_0, step_0, start_1, stop_1, step_1, ... \
                            Every axis needs to be explicit. The axis order is the one after the optional transpose")
