...
class FusionsCommand(NNToolShellBase):
    ...
    parser_fustions_exclusive.add_argument('--pow2',
                                           action='store_true',
                                           help='(deprecated: use --pow2_match_group) apply standard fusions for AutoTiler POW2 kernels')
    parser_fustions_exclusive.add_argument('--scale8',
                                           action='store_true',
                                           help='(deprecated: use --scaled_match_group) apply standard fusions for AutoTiler SQ8 kernels')

    parser_fustions_exclusive.add_argument('--pow2_match_group',
                                           action='store_true',
                                           help='apply standard fusions for AutoTiler POW2 kernels')
    parser_fustions_exclusive.add_argument('--scaled_match_group',
                                           action='store_true',
                                           help='apply standard fusions for AutoTiler SQ8 kernels')
    parser_fustions_exclusive.add_argument('--float_match_group',
                                           action='store_true',
                                           help='apply standard fusions for AutoTiler SQ8 kernels')
...
Carry out the default set of fusions on the graph"""
        ...
        try:
            ...
            elif args.pow2 or args.pow2_match_group:
                fusions_names = ['pow2_match_group']
            elif args.scale8 or args.scaled_match_group:
                fusions_names = ['scaled_match_group']
            elif args.float_match_group:
                fusions_names = ['float_match_group']
            else:
                self.perror(
