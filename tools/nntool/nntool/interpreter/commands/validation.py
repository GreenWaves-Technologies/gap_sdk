...
from nntool.utils.validation_utils import ValidateFromJSON, ValidateFromName, ValidateFromClass, ValidateFromVWWInstances
...
def validation_options(parser):
    ...
    parser_val_group.add_argument('--class_number', default=None, type=int,
...
def get_validator(args):
    if args.vww_instances_file:
        return ValidateFromVWWInstances(args.vww_instances_file,
                                        ...
                                        out_idx=args.prediction_step_idx)
    if args.label_json:
        ...
        try:
            ...
            for i, file_per_input in enumerate(glob_input_files(args.input_files, self.G.num_inputs)):
                ...
                res = validation.validate(
                    data, outputs, input_name=file_per_input[0])
                good_prediction = res.validated
                class_predicted, real_class, margin = res.result
