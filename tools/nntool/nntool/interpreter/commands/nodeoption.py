...
from nntool.generation.memory_device_info import (AT_L3_FLASH_DEVICES,
                                           AT_L3_RAM_DEVICES)
...
can be set refer to the autotiler documentation."""
        ...
        if args.parameter is None:
            nothing = True
            for node in nodes:
                node_options = self.G.node_options.get(node.name)
                if node_options:
                    nothing = False
        ...
        for node in nodes:
            ...
            try:
                if args.parameter in ["RNN_STATES_AS_INPUTS", "OUTPUT_C_STATE", "LSTM_OUTPUT_C_STATE"] and val:
                    if args.parameter == "RNN_STATES_AS_INPUTS":
                        node.set_states_as_inputs(self.G)
                    if args.parameter in ["OUTPUT_C_STATE", "LSTM_OUTPUT_C_STATE"]:
                ...
                self.G.node_options[node.name] = node_options
            except KeyError:
                self.pwarning(
                    f"{args.parameter} is not a valid parameter for node {node.name}")
