...
from nntool.graph.types import (NNEdge, ResizerNodeBase, BilinearResizerNode,
...
class InputResizerCommand(NNToolShellBase):
    ...
    def op_choices(self):
        return [fmt.lower() for fmt in ResizerNodeBase.SUPPORTED_OP_TYPES] + ['none']
...
def insert_resizer(G, out_edge, resize_op, from_shape):
    ...
    if resize_op == 'bilinear':
        resize_node = BilinearResizerNode(input_node.name + "_resizer", (old_in_dim.h, old_in_dim.w))
    elif resize_op == 'nearest':
        resize_node = NearestNeighborResizerNode(input_node.name + "_resizer", (old_in_dim.h, old_in_dim.w))
    else:
    ...
    if input_qrec:
        ...
        G.quantization[resize_node.name] = resizer_qrec
