...
@paramstype(SplitNode)
class GenSplitLastAxisParameters(GeneratorBase, InOutBindingsMixin):
    @classmethod
    ...
    def bindings_generator(cls, gen, node, qrec, in_eparams, out_eparams, cname) -> bool:
        ...
        bind_list = [GNodeArgEdge(in_par) for in_par in in_eparams] 
        bind_list += [GNodeArgEdge(out_par, "GNA_OUT") for out_par in out_eparams]
        gen.bindings.append(
    ...
    def kernel_generator(cls, gen, node, qrec, in_eparams, out_eparams, cname) -> bool:
        gen.kernels.append(SplitLastAxisKernel(cname, node, qrec))
        return True


class SplitLastAxisKernel(NewAutoTilerKernel):
    CALL_TEMPLATE = '''
// generator for {node_name}
CNN_SplitLastAxis_Generator("{cname}", {gen_ctrl}, {data_size}, {height}, {width1}, {width2}, {width3}, {width4}, KOP_SPLIT);
'''
    ...
    def __init__(self, cname, params, qrec, gen_ctrl=None):
        ...
        attrs = {
            ...
            'width3': widths[2],
            'width4': widths[3],
            'data_size': qrec.in_qs[0].dtype_bits//8
        }
