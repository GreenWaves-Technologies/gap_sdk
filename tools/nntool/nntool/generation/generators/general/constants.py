...
from nntool.generation.at_types.tc_arg_info import GlobalArgInfo, GlobalResetArgInfo, InputArgInfo
...
from nntool.graph.types.fusions import ConvFusionNode, LinearFusionNode, MatMulOpFusionNode
from nntool.utils.numpy_helpers import interleave, packbits

LOG = logging.getLogger(__name__)


@paramstype(ConstantInputNode)
class GenConstantInputParameters(GeneratorBase):
    @classmethod
    def globals_generator(cls, gen, node, qrec, pnode, fnode) -> bool:
        ...
        if not pnode.is_constant:
            ...
            if qtype.attr.concatenated_nodes:
                values = [value]
                concatenated_nodes = [gen.G[node_name]
                                      for node_name in qtype.attr.concatenated_nodes]
                concated_qrecs = [gen.G.quantization.get(pn.name)
            ...
            if qtype.attr.resize:
                padding = tuple((0, new-orig)
                                for orig, new in zip(*qtype.attr.resize))
            ...
            if qtype.attr.ne16_biases:
                ...
                if isinstance(to_node, (ConvFusionNode, LinearFusionNode, MatMulOpFusionNode)):
                    cnodes = to_node.contained_nodes()
                    quants = [gen.G.quantization[(
                        to_node.name, fnode.name)] for fnode in cnodes]
                    filter_qrec = quants[0]
                else:
                    filter_qrec = gen.G.quantization[to_node.name]
                if 'mul_biases_q' not in filter_qrec.cache:
                    raise ModelGenerationInternalError(
                        f"mul_biases_q not found in qrec for {to_node.name}")
