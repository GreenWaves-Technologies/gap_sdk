...
from nntool.generation.generators.generator_base import (GeneratorBase,
...
from nntool.utils.numpy_helpers import interleave

LOG = logging.getLogger(__name__)

from nntool.generation.generators.helpers.sigmoid_table import SIGMOID_TABLE
    ...
    def bindings_generator(cls, gen, node, qrec, in_eparams, out_eparams, cname) -> bool:
        ...
        if isinstance(i_state_eparams.creating_node, ConstantInputNode):
            reset_name = (i_state_eparams.creating_node.reset_name
                        if not node.rnn_states_as_inputs else
                        f"{node.name}_Reset")
        else:
            reset_name = None
        bindings = [
