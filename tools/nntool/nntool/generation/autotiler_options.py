...
AUTO_TILER_OPTIONS = [
        ...
        'descr': 'When all user kernel arguments can fit into given L1 memory promote them to buffer', 'default': True},
    {
        ...
        'descr': 'When all tiles of a user kernel argument across Input Features can fit into given L1 memory promote them to partial buffer', 'default': True},
    {
        ...
        'descr': 'Enable automatic cycle capture for each node of the graph', 'default': False},
    {
        ...
        'descr': 'When GRAPH_MONITOR_CYCLES is enabled, set the name of the C var array to receive results', 'default': 'AT_GraphPerf'},
    {
        ...
        'descr': 'Enable production of an array containing the name of each graph node', 'default': False},
    {
        ...
        'descr': 'When GRAPH_PRODUCE_NODE_NAMES is enamed, set the name of the C array receiving the names as strings', 'default': 'AT_GraphNodeNames'},
    {
        'name': 'GRAPH_PRODUCE_OPERINFOS', 'type': 'graph', 'var_type': bool, 'choices': [True, False],
        'descr': 'Enable production of number of operations/macs for each layer', 'default': False},
    {
        'name': 'GRAPH_PRODUCE_OPERINFOS_CVAR_NAME', 'type': 'graph', 'var_type': str, 'choices': None,
        'descr': 'When GRAPH_PRODUCE_OPERINFOS is enabled, set the name of the C array receiving mac infos for each node', 'default': 'AT_GraphOperInfosNames'},
    {
        'name': 'GRAPH_REORDER_CONSTANT_IN', 'type': 'graph', 'var_type': bool, 'choices': [True, False],
        'descr': 'Enable reordering of constant inputs in order to transform 2D accesses into 1D accesses', 'default': True},
    {
        ...
        'descr': 'If on, constant symbols executes from home location', 'default': False},
    {
        ...
        'descr': 'L3 flash type preferrer for for constant symbols', 'default': 'AT_MEM_L3_HFLASH'
    },
    {
    ...
    {
        'name': 'GRAPH_WARM_CONSTRUCT', 'type': 'graph', 'var_type': int, 'choices': (0, 1, 2, 3),
        'descr': 'Generate construct/destruct functions with the Warm option to only 1. allocate/deallocate L1 buffer, 2. separate alloc/dealloc of L2 dynamic memory, 3. fully flexible construct/destruct with separate allocation functionalities', 'default': 0
    },
    {
        'name': 'GRAPH_GROUP_WEIGHTS', 'type': 'graph', 'var_type': bool, 'choices': [True, False],
        'descr': 'Group together KerArg of the same type/size in the context of the single layer. For example weights and biases in RNN', 'default': False
    },
    {
        'name': 'GRAPH_ASYNC_FORK', 'type': 'graph', 'var_type': bool, 'choices': [True, False],
        'descr': 'ONLY AVAILABLE IN GAP9: Generate user kernel code with async fork option', 'default': False
    },
    {
        'name': 'GRAPH_DUMP_GRAPH_OUTPUTS', 'type': 'graph', 'var_type': bool, 'choices': [True, False],
        'descr': 'Trace graph output tensors at inference time', 'default': False
    },
]
...
for name, default in DEFAULT_GEN_OPTS.items():
    if name in DEFAULT_GEN_OPTS_DESCRIPTIONS:
        DEFAULT_GEN_OPTS_DESCRIPTIONS[name]['default'] = default

for opt in AUTO_TILER_OPTIONS:
    _NAME = opt['name']
    if not _NAME.startswith('GRAPH_'):
        continue
    # legacy mess
    _NAME = _NAME.lower()
    _SHORT_NAME = _NAME[len('GRAPH_'):]
    if _SHORT_NAME in DEFAULT_GEN_OPTS_DESCRIPTIONS:
        del DEFAULT_GEN_OPTS_DESCRIPTIONS[_SHORT_NAME]
    DEFAULT_GEN_OPTS_DESCRIPTIONS[_NAME] = {
        'type': opt['var_type'],
        'descr': opt['descr'],
        'default': opt['default'],
        'choices': opt['choices'],
