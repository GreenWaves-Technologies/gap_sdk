...
def read_float(buffer, offset, bit_size):
    ...
    raise FlexbufferParseException(
        "Invalid bit size for flexbuffer float: %d" % bit_size)
...
def read_array(buffer, offset, length, bit_size, packed_type, decode_strings):
    ...
    for i in range(length):
        ...
        arr.append(read_buffer(buffer, item_offset,
                               bit_size, packed_type, decode_strings))
    return arr
...
def read_buffer(buffer, offset, parent_bit_size, packed_type, decode_strings):
    ...
    if value_type == 0x5:
        ...
        while read_int(buffer, str_offset + size, 0) != 0:
            size_byte_size <<= 1
            size_bit_size += 1
            size = read_uint(buffer, str_offset -
                             size_byte_size, size_bit_size)
        return read_string(buffer, str_offset, size, decode_strings)
    ...
    if value_type == 0x9:
        length = read_uint(buffer, read_indirect(
            buffer, offset, parent_bit_size) - byte_size, bit_size)
        keys_offset = read_indirect(
            buffer, offset, parent_bit_size) - (byte_size * 3)
        ...
        for i in range(length):
            ...
            key = read_buffer(buffer, key_offset, key_bit_size,
                              (0x4 << 2) | key_bit_size, decode_strings)
            value_offset = values_offset + i * byte_size
            value_packed_type = read_uint(buffer, packed_types_offset + i, 0)
            value = read_buffer(buffer, value_offset, bit_size,
                                value_packed_type, decode_strings)
            obj[key] = value
        return obj
    if value_type == 0xa:
        length = read_uint(buffer, read_indirect(
    ...
    if value_type in [0xb, 0xc, 0xd, 0xe, 0xf, 0x24]:
        length_offset = read_indirect(
            buffer, offset, parent_bit_size) - byte_size
    ...
    raise FlexbufferParseException(
        "Invalid flexbuffer value type %r" % value_type)
