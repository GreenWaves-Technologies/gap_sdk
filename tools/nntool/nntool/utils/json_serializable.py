...
class JsonSerializable(ABC):
    ...
    def _encapsulate(self):
        raise NotImplementedError(
            "JsonSerializable  must implement _encapsulate()")

    @abstractclassmethod
    def _dencapsulate(cls, val):
        raise NotImplementedError(
            "JsonSerializable  must implement _dencapsulate()")
    ...
    def from_dict(cls: 'JsonSerializable', val):
        assert val['__type'] == 'JsonSerializable', 'not a JsonSerializable'
        # TODO: This may be deleted in the future. Backwards compatibility with NodeID 
        if val['__module_name'] == 'utils.node_id' and val['__class_name'] == 'NodeId':
            val = val['__contents'].split('###')
            if not val[1]:
                return val[0]
            return tuple(val)
...
class JsonSerializableStateDecoder(json.JSONDecoder):
    def __init__(self, *args, object_hook=None, **kwargs):
        if object_hook is None:
            super(JsonSerializableStateDecoder, self).__init__(
...
# pylint: disable=no-self-use, method-hidden
    def object_hook(self, obj):
        if '__type' in obj:
            if obj['__type'] == 'set':
                return set(obj['__contents'])
            if obj['__type'] == 'dict':
                json_module = importlib.import_module(obj['__module_name'])
                json_class = getattr(json_module, obj['__class_name'])
                return json_class(
                    (
                        tuple(elem[0]) if isinstance(
                            elem[0], list) else elem[0],
                        elem[1])
                    for elem in obj['__contents'])
