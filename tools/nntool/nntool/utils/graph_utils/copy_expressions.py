...
import numpy as np
from nntool.graph.types import (ConcatNode, FusionInputNode,
                         FusionOutputNode, NNNodeBase, SplitNode,
                         StridedSliceNode,
                         TransposeNode)
...
def explore_node(subg, node, state, out_slice=None, in_slice=None):
    ...
    if isinstance(node, ConcatNode):
        ...
        explore_edges(src_slice_sets, dest_slice_sets, state, node, subg)
    elif isinstance(node, StridedSliceNode):
...
def walk_down(
        subg: GraphView,
        node: NNNodeBase,
        node_slices: Mapping[NNNodeBase, Sequence[SlicedTensor]]):
    ...
    if isinstance(node, FusionInputNode):
        ...
        if any(val is None for val in dim_slices):
            return
        if isinstance(node, TransposeNode) and node.transpose:
...
class CopySet():
    def __init__(self, elements: Sequence[Tuple[NNNodeBase, SlicedTensor]]) -> None:
        self.elements = elements
