...
from nntool.quantization.qtype import DTYPE_GAP_CTYPE

from .function import Function
from .symbol import (Constant, QRecBase, QuantizedConstant, Rational, Symbol, Variable, c_headers,
...
class NoOP(Function):
    def _impl(self, *args, **kwargs):
        return self.contents[0](*args, **kwargs)
...
class LShift(Function):
    ...
    def _eval(self, *args, **kwargs):
        ...
        # is also a Q15
        if isinstance(sym, LShift):
            return LShift(sym.contents[0], Add(sym.contents[1], args[1], dtype=self.dtype), dtype=self.dtype).resolve(**kwargs)
        if isinstance(sym, Norm):
            return Norm(sym.contents[0], Sub(sym.contents[1], args[1], dtype=self.dtype), dtype=self.dtype).resolve(**kwargs)
...
class Cosh(Function):
    ...
    def _impl(self, *args, **kwargs):
        return np.cosh(args[0], dtype=self.dtype)

    def _py_expr(self, *args, **kwargs):
        return "np.cosh(%s)" % (args[0],)

    def _c_expr(self, *args, **kwargs):
        return "cosh(%s)" % (args[0],)
...
class Erf(Function):
    ...
    def _impl(self, *args, **kwargs):
        return np_erf(args[0]).astype(np.float32)

    def _py_expr(self, *args, **kwargs):
        return f"np_erf({args[0]}).astype(np.float32)"
...
class Cast(Function):
    def __init__(self, *args, dtype=None, cast_dtype=None, **kwargs):
        assert cast_dtype is None or cast_dtype == dtype
    ...
    def _eval(self, *args, **kwargs):
        ...
        if args[0].dtype == self._cast_dtype:
            return args[0]
        return Cast(args[0], dtype=self.dtype)
...
class CompoundFunction(Function):
    ...
    def _equivalent(self, other):
        return (isinstance(other, CompoundFunction) and
                self.inner_function.resolve().equivalent(other.inner_function.resolve()))

    def _extract_constants(self, other, constants=None) -> bool:
        if not isinstance(other, CompoundFunction):
            raise ValueError(
                f'extract_constants: {other} is not a compound function')
        return self.inner_function.extract_constants(other.inner_function, constants=constants)

    def substitute(self, find_sym, sub_sym, update=False):
        return self.inner_function.substitute(find_sym, sub_sym, update=update)
...
@nargs(2)
@c_headers('"Gap.h"')
@copy_props('clip_signed')
class ClipBits(Function):
    def __init__(self, *args, clip_signed=True, **kwargs):
        self._clip_signed = clip_signed
        super().__init__(*args, **kwargs)

    def calculate_shape(self):
        self._shape = self._contents[0].calculate_shape()
        return self._shape

    @property
    def clip_signed(self):
        return self._clip_signed

    def get_clip_min_max(self, bits):
        if isinstance(bits, Constant):
            bits = bits.value
        max_val = int(math.pow(2, bits))
        if self._clip_signed:
            min_val = -max_val
            max_val -= 1
        else:
            min_val = 0
    ...
    def _impl(self, *args, **kwargs):
        clip_min, clip_max = self.get_clip_min_max(args[1])
        return np.maximum(np.minimum(args[0], clip_max, dtype=self.dtype), clip_min, dtype=self.dtype)

    def _py_expr(self, *args, **kwargs):
        clip_min, clip_max = self.get_clip_min_max(args[1])
        return f"np.maximum(np.minimum({args[0]}, {clip_max}), {clip_min})"

    def _c_expr(self, *args, **kwargs):
        if self._clip_signed:
            return f"gap_clip(({args[0]}), ({args[1]}))"
        return f"gap_clipu(({args[0]}), ({args[1]}))"

    def __repr__(self) -> str:
        clip_min, clip_max = self.get_clip_min_max(self.contents[1])
        return f"Clip({self.contents[0]}, {clip_min}, {clip_max})"


@copy_props('clip_dtype')
class Clip(ClipBits):
    def __init__(self, *args, clip_dtype=None, clip_signed=None, **kwargs):
        self._clip_dtype = clip_dtype
        clip_bits = math.ceil(math.log2(np.iinfo(clip_dtype).max))
        clip_signed = np.issubdtype(clip_dtype, np.signedinteger)
        super().__init__(args[0],
                         QuantizedConstant(np.atleast_1d(clip_bits).astype(np.int8)),
                         clip_signed=clip_signed,
                         **kwargs)
...
class Norm(Function):
    ...
    def _eval(self, *args, **kwargs):
        sym = args[0]
        if isinstance(sym, Norm):
            return Norm(sym.contents[0], Add(args[1], sym.contents[1], dtype=np.int32), dtype=np.int32).resolve(**kwargs)
        if isinstance(sym, LShift):
            return Norm(sym.contents[0], Sub(args[1], sym.contents[1], dtype=np.int32), dtype=np.int32).resolve(**kwargs)
        return super()._eval(*args, **kwargs)
