...
class Function(VariableContainer, Symbol):
    ...
    @property
    def dtypes_in_use(self) -> Set[type]:
        ...
        for elem in self.contents:
            if isinstance(elem, Function):
                res |= elem.dtypes_in_use
            elif elem.dtype is not None:
                res.add(elem.dtype)
        return res

    def _eval(self, *args, **kwargs):
        if all(isinstance(arg, Constant) for arg in args):
            val = self._impl(*[arg.value for arg in args], **kwargs)
            if val.dtype != self.dtype:
                raise ArithmeticError(
                    f"Expression {self.name} evaluated to incorrect dtype")
            new_kwargs = {
                'dtype': val.dtype,
                'shape': val.shape,
                'name': self.name,
    ...
    def _extract_constants(self, other, constants=None) -> bool:
        for scont, ocont in zip(self.contents, other.contents):
            scont.extract_constants(ocont, constants=constants)
    ...
    def _equivalent(self, other) -> bool:
        ...
        if len(self.contents) != len(other.contents):
            return False
        if not all(scont.equivalent(ocont) for scont, ocont in zip(self.contents, other.contents)):
            return False
        if not all(getattr(self, prop, None) == getattr(other, prop, None) for prop in self.COPY_PROPS):
            return False
        return True
    ...
    def calculate_shape(self):
        self._shape = self.broadcast(
            *(elem.calculate_shape() for elem in self._contents))
        return self._shape
    ...
    def c_block(self, code_block=None, tags=None, with_comment=False, **kwargs):
        ...
        if tags is not None:
            ...
            if self.tag:
                ...
                name = tags.get(
                    self, f'{self.ctype} {self.SYMBOL_PREFEX}{self.name}')
                if isinstance(name, tuple):
                    name = name[0].c_expr(
