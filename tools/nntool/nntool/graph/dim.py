...
class Dim():
    def __init__(self, shape=None, names=None, is_ordered=False, is_unknown=False):
        set_shape = shape if shape is not None else [
        ] if names is None else [None] * len(names)
    ...
    def _update_unknown(self):
        object.__setattr__(self, '_is_unknown', any(
            elem is None for elem in self._shape))
    ...
    def apply_naming_hints(self, hint):
        ...
        if len(self._shape) != len(hint):
            if len(self.layout_shape) == 1 and 'c' in hint:
                ...
                self._shape = [self.layout_shape[0]
                               if elem == 'c' else 1 for elem in hint]
            else:
                raise MissMatchedInputsError()
        # pylint: disable=protected-access
    ...
    def impose_order(self, order) -> 'Dim':
        if not object.__getattribute__(self, '_is_unknown'):
            object.__setattr__(
                self, '_shape', [getattr(self, k) for k in order])
        if self.is_named:
    ...
    def makeshape(self, order):
        ...
        for elem in order:
            if isinstance(elem, str):
                ...
                newshape.append(
                    reduce(lambda x, y: x * getattr(self, y), elem, 1))
            else:
                raise TypeError(
                    "elements of order should be k or list of keys")
        return newshape
    ...
    def combine(dims: Iterable, axis) -> 'Dim':
        ...
        if isinstance(axis, int):
            if axis >= len(base.shape):
                ValueError(
                    f'axis {axis} is invalid for a shape of len {len(base.shape)}')
            cnt = base.shape[axis]
            for i in range(1, len(dims)):
                dim = dims[i]
    ...
    def __repr__(self) -> str:
        if self.is_named:
            dims = ','.join(f'{k}={getattr(self, k)}' for k in self.keys)
            return f"{self.__class__.__name__}.named(order={self.is_ordered}, {dims})"
        return repr(tuple(self.shape))
...
class PadDim(Dim):
        ...
        if not args:
            ...
            if len(args) == 1:
                ...
                super().__init__([args[0], args[0], args[1],
                                  args[1]], PAD_DIMS, is_ordered=True)
            elif len(args) == 4:
                super().__init__([args[0], args[1], args[2],
                                  args[3]], PAD_DIMS, is_ordered=True)
            else:
        ...
        if l > 0:
            ...
            if r > l:
                ...
                if l != half_pad or r != total - half_pad:
                    raise ValueError(
                        "padding incompatible with autotiler - not balanced r>l")
                return [False, False, False, True]
            # should match balanced left
            if r != half_pad or l != total - half_pad:
                raise ValueError(
                    "padding incompatible with autotiler - not balanced l>r")
            if r == l:
    ...
    def pad_compatibility_reduce(cls, *pad_compatibilities, err_msg=None):
        ...
        if not any(reduction):
            if err_msg:
                raise ValueError(
                    "padding is incompatible with autotiler - " + err_msg)
            return None
        return reduction
    ...
    def calculate_same_h_w(self, in_dim, filt, stride, dilation=None):
        ...
        if dilation is None:
            ...
            pad_along_height = max(
                (out_height - 1) * stride.h +
                ((dilation.h - 1) * (filt.h - 1) + filt.h) - in_dim.h,
                0)
            pad_along_width = max(
                (out_width - 1) * stride.w +
                ((dilation.w - 1) * (filt.w - 1) + filt.w) - in_dim.w,
                0)
    ...
    def calculate_same(self, in_dim, filt, stride, dilation=None) -> Dim:
        ...
        pad_along_height, pad_along_width = self.calculate_same_h_w(
            in_dim, filt, stride, dilation)
    ...
    def has_at_pad(self) -> bool:
        ...
        raise AttributeError(
            "Padding is not same so not compatible with AutoTiler")

    def __str__(self):
        if self._is_unknown:
            return 'unknown'
        shape = [(self.shape[idx], self.shape[idx+1])
                 for idx in range(0, len(self.shape), 2)]
        return "x".join(str(dim) for dim in shape)

    def __repr__(self) -> str:
        if self.is_unknown:
            return f'PadDim(same_type={self.same_type!r})'
        return f'PadDim({self.t}, {self.b}, {self.l}, {self.r}, same_type={self.same_type!r})'
...
class Conv2DFilterDim(Dim):
    ...
    def __init__(self, h, w, out_c, in_c=None, order=None):
        super().__init__([out_c, in_c, h, w],
                         DEFAULT_CONVFILTER_DIMS.copy(), is_ordered=True)
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}({self.h}, {self.w}, {self.out_c}, "
                f"in_c={self.in_c}, order={self.keys})")
...
class FcFilterDim(Dim):
    ...
    def actual_order(self):
        ...
        assert out_c_first or out_c_idx == (
            len(self.order) - 1), "out_c should be first or last"
        if out_c_first:
            return ['out_c', 'sz']
        else:
            return ['sz', 'out_c']
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}({self.out_c}, {self.in_c}, "
                f"order={self.keys})")
...
class Dim2D(Dim):
    ...
    def __init__(self, *args, order=None):
        if len(args) == 1:
            super().__init__([args[0], args[0]],
                             DEFAULT_2DDIMS.copy(), is_ordered=True)
        elif len(args) == 2:
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}({self.h}, {self.w}, "
                f"order={self.keys})")
