...
from nntool.quantization.compression.compress import CompressionError, compress
from nntool.quantization.qtype import QType

from .base import NNNodeRef, cls_op_name
from .input_output import InputNNNodeBase

LOG = logging.getLogger(__name__)

@cls_op_name('constant')
class ConstantInputNode(InputNNNodeBase):

    def __init__(self, *args, is_mutated=False,
        ...
        super(ConstantInputNode, self).__init__(
    ...
    def attrs(self) -> dict:
        ...
        super_attrs.update({k: getattr(self, k) for k in [
            "is_mutated", "is_intermediate", "always_copy",
        ] if getattr(self, k)})
        return super_attrs
    ...
    def write_python_description(self, code_block: StreamCodeBlock, indent=False, tensors=None) -> str:
        ...
        code_block.write_lines(attrs)
        if self.qtype:
            code_block.write('qtype=', nl='')
            self.qtype.dump(tensors, (self.name, 'qtype'), code_block, indent=False, nl='')
            code_block.write(',', indent=False)
        code_block.write(f'value={repr(DumpTensorRef(self.name))}')
        tensors[self.name] = self.value
        code_block.deindent()
        code_block.write(')', nl='')
...
#pylint: disable=arguments-differ
    ...
    def compress_value(self, qtype=None, bits=None, min_qsnr=None, force_sparse=False,
        ...
        if qtype:
            if (qtype.attr.no_compression or np.any(qtype.offset) or np.any(qtype.attr.interleaved_values)):
