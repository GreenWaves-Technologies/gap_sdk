...
from nntool.graph.types.base import NNEdge, cls_op_name, nargs


from .rnn import RNNNodeBase
...
class LSTMNode(RNNNodeBase):
    ...
    def __init__(self, name, *args, cell_clip=0.0, proj_clip=0.0, **kwargs):
        super(LSTMNode, self).__init__(name, *args, **kwargs)
        self.cell_clip = cell_clip
        self.proj_clip = proj_clip
        self.at_options.valid_options['LSTM_OUTPUT_C_STATE'] = int
        self.at_options.valid_options['OUTPUT_C_STATE'] = int
    ...
    def set_c_state_as_output(self, G):
        ...
        lstm_qrec = G.quantization and G.quantization.get(self.name)
        if lstm_qrec:
            ...
            G.quantization[output_c_state.name] = c_state_q

        G.add_edge(NNEdge(self, output_c_state, from_idx=1))
        self.output_c_state = True
        G.add_dimensions()

    @property
    def output_c_state(self):
        if hasattr(self.at_options, 'output_c_state'):
            return self.at_options.output_c_state == 1
        return False

    @output_c_state.setter
    def output_c_state(self, val):
        self.at_options.output_c_state = 1 if val else 0
    ...
    def get_output_size(self, in_dims):
        output_dims = super(LSTMNode, self).get_output_size(in_dims)
        if self.output_c_state:
