...
from nntool.graph.manipulations.eliminate_transposes.transpose_helpers import apply_transpose
...
class AdjusterBase():
    ...
    def apply_input_trans(self, G, node, trans: list, index=None):
        ...
        for idx in range(start, end):
            params = TransposeNode(G.unique_name(
                f"{node.name}_trans_in{idx}"), transpose=trans)
            ...
            if node.in_dims_hint:
                node.in_dims_hint[idx] = apply_transpose(
                    node.in_dims_hint[idx], trans)
            nid = node.name
            if G.quantization and nid in G.quantization:
                G.quantization.copy_qrec(node, 'in', idx, params)

    def apply_output_trans(self, G, node, trans: list, index=None, check_unused_output=False):
        if index is None:
            start = 0
        ...
        indexed_out_edges = G.indexed_out_edges(node)
        for idx in range(start, end):
            # check if an output goes only to an OutputParameters with not_used set
            if check_unused_output:
                edges = indexed_out_edges[idx]
                if (len(edges) == 1 and isinstance(edges[0].to_node, OutputNode) and
                        edges[0].to_node.not_used):
                    continue
            params = TransposeNode(G.unique_name(
                f"{node.name}_trans_out{idx}"), transpose=trans)
            G.insert_node_after(
                node,
                params,
                from_idx=idx,
            ...
            if node.out_dims_hint:
                node.out_dims_hint[idx] = apply_transpose(
                    node.out_dims_hint[idx], self.invert(trans))
            nid = node.name
            if G.quantization and nid in G.quantization:
    ...
    def verify_chw(node, names: list, has_batch=False):
        if len(names) != 3 and not (has_batch and len(names) == 4):
        ...
        if not all(elem in names for elem in expected_dims):
            LOG.error("Conv %s: input has strange elements %s",
                      node.name, names)
            raise AdjusterException()
    ...
    def get_all_handlers(cls, opts):
        ...
        for handler_cls in get_all_subclasses(cls):
            ...
            for params_cls in handler_cls.HANDLES[0]:
                ...
                handlers_for.append(
                    (handler_cls(opts), handler_cls.HANDLES[1]))
        return handlers
    ...
    def get_output_qrec_class(G):
        ...
        qrec = G.quantization.get(outs.name, None)
        if not qrec:
    ...
    def check_quantization(self, G, node, reshape, direction='in'):
        if G.quantization:
            ...
            node_qrec = G.quantization[node.name]
            qtype = getattr(node_qrec, f'{direction}_qs')[0]
            G.quantization[reshape.name] = qclass(
