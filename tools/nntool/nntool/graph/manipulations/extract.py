...
from ..types import NNNodeBase, SingleInputAndOutputMixin, NNEdge

LOG = logging.getLogger(__name__)


def extract_node(G: NNGraph, keep_node: NNNodeBase):
    if not isinstance(keep_node, SingleInputAndOutputMixin):
        raise NotImplementedError(
            "exclude only works with single input and output nodes at present")
    LOG.debug("extracting node %s into new graph", keep_node.name)
    ...
    keep_nodes = [edge.from_node for edge in in_edges
                  if edge is not None and isinstance(edge.from_node, ConstantInputNode)] + [keep_node]
