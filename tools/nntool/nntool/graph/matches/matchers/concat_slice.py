...
from nntool.graph.types import ConcatNode, NNEdge, ReshapeNode, StridedSliceNode
from nntool.graph.types.misc import (NoOPNode)
from nntool.utils.compatible_transposes import find_combination
...
def validate_slice(node: StridedSliceNode):
...
def up_from_slice(G, edge, axis, shape, remove_nodes=None, removing=True, reshape=None):
    ...
    if isinstance(node, ConcatNode):
        ...
        elif not isinstance(node, NoOPNode):
            return [None] * 4
...
class ConcatSliceMatch(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs) -> bool:
        ...
        concat_slice_edges = {}
        for slice_node in G.nodes(node_classes=StridedSliceNode):
