...
class FusionMatch():
    ...
    def add_node(self, params):
        if isinstance(params, PadNode):
            if self.pad:
                return None
            self.order.append(params)
            self.pad = params
            return self
        elif isinstance(params, MatrixAddNode):
            if self.add:
                return None
            self.order.append(params)
            self.add = params
            return self
        elif isinstance(params, ActivationNodeBase):
            if self.active:
                return None
            self.order.append(params)
            self.active = params
    ...
    def fusion_type(self):
        return '_'.join(['pad' if isinstance(params, PadNode)
                         else 'active' if isinstance(params, ActivationNodeBase)
...
class MatchPadAddAct(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        for pad_node in [params for params in G.nodes() if isinstance(params, PadNode)]:
