...
from nntool.graph.types import MatrixMulNode, MatScaleFusionNode
from numpy.core.fromnumeric import prod
from nntool.utils.graph import Edge, GraphView, Node
from nntool.utils.graph_matcher import (EdgeMatch, GraphMatcher, MatchFinishSuccess,
                                 MatchNodeByClass, NodeMatch)

from ..matcher import Matcher, description, match_name, run_before, needs_valid_dimension
...
class MatScalePairMatchFactory():
    class MatchMatMulTwice(MatchNodeByClass):
        def __init__(self, factory):
            super(MatScalePairMatchFactory.MatchMatMulTwice,
                  self).__init__(MatrixMulNode)
    ...
    class MatScaleEdgeMatch(EdgeMatch):
        def __init__(self, factory, *args, **kwargs):
            super(MatScalePairMatchFactory.MatScaleEdgeMatch,
        ...
        def match_down(self, G, edge: Edge, state):
            if not isinstance(edge.to_node, MatrixMulNode):
                return False
    ...
    def get_matcher(self):
        ...
        node_1 = MatScalePairMatchFactory.MatchMatMulTwice(MatrixMulNode)
        node_2 = MatScalePairMatchFactory.MatchMatMulTwice(MatrixMulNode)
...
class MatScaleNodeMatch(NodeMatch):
    ...
    def match(self, G, node: Node, state):
        if not isinstance(node, MatrixMulNode) or self._has_matched:
            return False
        scalars = set()
        vectors = set()
        ...
        if len(scalars) == 2:
            state['match'] = {'type': 'scalar',
                              'inputs': [scalars.pop(), scalars.pop()]}
            return True
        if len(scalars) == 1:
            vectors = vectors - scalars
            state['match'] = {'type': 'scalar',
                              'inputs': [vectors.pop(), scalars.pop()]}
            return True
        if len(vectors) == 2:
            state['match'] = {'type': 'vector',
                              'inputs': [vectors.pop(), vectors.pop()]}
            return True
        if len(vectors) == 1:
            node_edges = node_edges - vectors
            state['match'] = {'type': 'vector', 'inputs': [
                node_edges.pop(), vectors.pop()]}
            return True
        self._has_matched = False
        return False
...
@match_name("fuse_matscale_pair")
@description('Finds pair of Matrix piecewise multiplies and checks if they are compatible with a MatScale kernel')
@needs_valid_dimension(True)
@run_before('fuse_matscale')
class FuseMatScalePair(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        for frag, match in fac.get_matcher().match_graph(G):
            match_edges = [G.indexed_in_edges(
                node.name)[idx] for node, idx in match]
            ...
            input_mapping = MatScaleFusionNode.get_mapping_from_edges(
                match_edges)

            fnode = MatScaleFusionNode(
                "{}_{}_fusion".format(first_node.name, last_node.name),
                fusion_type="vec_scalar", subgraph=frag,
                input_mapping=MatScaleFusionNode.convert_input_mapping(input_mapping))
...
@match_name("fuse_matscale")
@description( 'Finds Matrix piecewise multiply and checks if it is compatible with a MatScale kernel')
