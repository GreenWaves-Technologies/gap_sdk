...
from nntool.graph.types import NNEdge, ReverseNode, RNNNodeBase
...
class MatchReversedRnn(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        reverse_nodes = set([node for node in G.nodes() if (isinstance(node, ReverseNode) and
        ...
        for reverse_node in reverse_nodes:
            ...
            for edge in G.out_edges(rnn_edge.to_node.name):
                if not isinstance(edge.to_node, ReverseNode):
                    continue
                if edge.from_idx != 0:
