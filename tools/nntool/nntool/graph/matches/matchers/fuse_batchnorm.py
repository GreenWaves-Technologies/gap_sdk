...
import numpy as np
from nntool.graph.types import (ConstantInputNode, MatMulOpNode, MatMulTransposedNode,
                         NNEdge)
from nntool.graph.types.conv2d import BatchNormalizationNode
...
class FuseBatchnorm(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        nodes = []
        for node in G.nodes(node_classes=BatchNormalizationNode):
            in_node = G.indexed_in_edges(node)[0].from_node
            if isinstance(in_node, MatMulOpNode):
        ...
        for bn_node, filt_node in nodes:
            ...
            if not isinstance(weights_node, ConstantInputNode):
                continue
            weights = weights_node.dqvalue
            if len(w_bn) > 1:
                if not isinstance(filt_node, MatMulTransposedNode):
            ...
            if len(w_bn) > 1 and not isinstance(filt_node, MatMulTransposedNode):
