...
from nntool.graph.types import GlobalPoolingNodeBase, NNEdge, ReshapeNode
...
class CombineReductions(Matcher):
    ...
    def reductions(G, node, first_node=None):
        if first_node is None:
            first_node = node
        if isinstance(node, GlobalPoolingNodeBase) and node.pool_type == first_node.pool_type:
    ...
    def _match(self, G: GraphView, **kwargs):
        nodes = list(G.nodes(node_classes=GlobalPoolingNodeBase))
        ...
        while nodes:
            ...
            if G.quantization:
                last_qrec = G.quantization[node_group[-1].name]
                G.quantization[new_node.name].out_qs = last_qrec.out_qs
