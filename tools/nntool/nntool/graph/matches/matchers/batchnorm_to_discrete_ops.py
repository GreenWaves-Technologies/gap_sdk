...
from nntool.graph.types.tensor_arithmetic import MatrixMulNode
...
class FuseBatchnorm(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        for bn_node in G.nodes(node_classes=BatchNormalizationNode):
            w_bn = bn_node.scale / \
            ...
            mul_params = MatrixMulNode(
                G.unique_name(f"{bn_node.name}_mul"))
            add_params = MatrixAddNode(
            ...
            scale_node = ConstantInputNode(G.unique_name(f"{bn_node.name}_scale"), value=w_bn.reshape(
                broadcasted_shape), dims=Dim.unnamed(broadcasted_shape))
            bias_node = ConstantInputNode(G.unique_name(f"{bn_node.name}_bias"), value=b_bn.reshape(
                broadcasted_shape), dims=Dim.unnamed(broadcasted_shape))
