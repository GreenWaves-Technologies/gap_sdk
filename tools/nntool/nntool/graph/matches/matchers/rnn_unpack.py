...
from nntool.graph.types import (ConcatNode, NNEdge, ReshapeNode,
                         RNNNodeBase, SplitNode,
...
class MatchRnnUnpack(Matcher):
    ...
    def find_unpack(self, G, node, first=True, out_shape=None, in_idx=None):
        if isinstance(node, RNNNodeBase):
            if not first:
                return []
        ...
        elif isinstance(node, (StridedSliceNode, SplitNode)):
    ...
    def validate_slices(self, G, rnn_nodes):
        ...
        for rnn_unpack in rnn_nodes:
            ...
            if isinstance(unpack_node, StridedSliceNode):
            ...
            elif isinstance(unpack_node, SplitNode):
    ...
    def all_paths_lead_to_rnn(self, G, node, first=True):
        if isinstance(node, (StridedSliceNode, SplitNode)):
            if not first:
                return False
        elif isinstance(node, RNNNodeBase):
        ...
        elif not isinstance(node, (TransposeNode, ConcatNode, ReshapeNode)):
            return False
    ...
    def _match(self, G: GraphView, **kwargs):
        rnn_nodes = [self.find_unpack(G, node)
                     for node in G.nodes()
                     if isinstance(node, RNNNodeBase) and node.n_output_cells > 1]
        ...
        for unpack_node, rnn_unpacks in rnn_nodes_by_slice.items():
            ...
            changes_shape = unpack_node.changes_shape if isinstance(
                unpack_node, StridedSliceNode) else False
            ...
            if changes_shape:
                reshape = ReshapeNode(unpack_node.name + '_reshape',
                ...
                if G.quantization:
                    G.quantization[reshape.name] = G.quantization[unpack.name]
            else:
                for out_edge in out_edges:
                    G.add_edge(NNEdge(from_node=in_edge.from_node, to_node=out_edge.to_node,
                                      from_idx=in_edge.from_idx, to_idx=out_edge.to_idx))
            if G.quantization:
                del G.quantization[unpack_node.name]
