...
from nntool.graph.types import (ConcatNode, CopyNode, InputNode,
                         NNEdge, ReshapeNode, NNNodeBase)
from nntool.utils.graph import GraphView, Node
...
def traverse_to_concat(G, edge, subgraph, edge_path=None):
    ...
    elif isinstance(node, (CopyNode, ReshapeNode)):
...
#         elif isinstance(edge.from_node, (CopyNode, ReshapeParameters)):
...
class CombineConcats(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        concats = set(G.nodes(node_classes=ConcatNode))
        while concats:
            concat = concats.pop()
            if concat.axis != 0:
                continue
            subgraph = find_concats_up(G, concat)
            found = set(subgraph.nodes(node_classes=ConcatNode))
            if len(found) <= 1:
                continue
            LOG.info(
