...
from nntool.graph.types import (ConstantInputNode, MatrixAddNode,
                         MatrixMulNode, NNEdge, TransposeNode, ReshapeNode)
from nntool.graph.types.tensor_arithmetic import MatMulOpNode, MatMulTransposedNode
...
def reverse_matmul(G: GraphView, params):
    ...
    in_nodes = []
    for idx in range(2):
        tin_params = TransposeNode(G.unique_name(
    ...
    tout_params = TransposeNode(G.unique_name(
...
class MatchExternalBiasMatmul(Matcher):
    ...
    def _match(self, G: GraphView, **kwargs):
        ...
        for params in G.nodes(node_classes=MatMulOpNode):
            matmul = params
            seen_reshape = []
            while True:
                ...
                if not isinstance(op_node, (MatrixAddNode, MatrixMulNode)):
                    break
                ...
                if isinstance(op_node, MatrixAddNode):
                    if has_bias:
                    ...
                    consts = [isinstance(
                        edge.from_node, ConstantInputNode) for edge in params_in]
                    if not any(consts):
                        break
