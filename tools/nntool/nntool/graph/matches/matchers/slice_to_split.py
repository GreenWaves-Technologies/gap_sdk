...
from nntool.graph.manipulations.eliminate_transposes.transpose_helpers import \
    reverse_transpose
from nntool.graph.types import (NNEdge, OutputNode, ReshapeNode,
...
from ..matcher import Matcher, description, groups, match_name, run_before, run_adjust_on_match, run_again_on_match
...
@groups('*')
@run_adjust_on_match
@run_again_on_match('split_concat', 'concat_split')
class SliceToSplitMatch(Matcher):
    ...
    def slice_to_split(G, slice_nodes, slices):
        ...
        if axis_slice[0] > 0:
            ...
            oparams = OutputNode(
                G.unique_name(f'{slice_node.name}_unused{0 if two_unused else ""}'),
                not_used=True)
        ...
        if axis_slice[1] < axis_dim:
            ...
            oparams = OutputNode(
                G.unique_name(f'{slice_node.name}_unused{1 if two_unused else ""}'),
                not_used=True)
            oparams.at_options.allocate = 1
            outs.append(
                ((oparams, 0),))
        ...
        if axis != 0 or (len(outs) == 1 and axis != len(in_dims) - 1):
            LOG.warning('adjust needs to be rerun')
        split_params = SplitNode(slice_node.name, act_slices=act_slices,
    ...
    def _match(self, G: GraphView, **kwargs) -> bool:
        ...
        slices_by_origin = {}
        for slice_node in [node for node in G.nodes() if isinstance(node, StridedSliceNode)]:
            in_edge = G.in_edges(slice_node.name)[0]
        ...
        for in_edge, slice_nodes in slices_by_origin.items():
            ...
            if reshape_from:
                params = ReshapeNode(
                    G.unique_name(name_prefix + '_reshape'),
                    old_shape=Dim.unnamed(reshape_from), shape=Dim.unnamed(reshape_to))
                G.add_edge(NNEdge(from_node=in_node,
                                  to_node=params, from_idx=in_idx))
                in_node = params
                in_idx = 0
                ...
                for out_edge in out_edges:
                    ...
                    if transpose_from:
                        ...
                        G.add_edge(NNEdge(from_node=from_node,
                                          to_node=params, from_idx=out_idx))
                        out_idx = 0
                    if node.changes_shape:
                        from_node = params
                        params = ReshapeNode(
                            G.unique_name(name_prefix + f'_reshape{idx}'), shape=node.out_shape)
                        G.add_edge(NNEdge(from_node=from_node,
                                          to_node=params, from_idx=out_idx))
                        out_idx = 0
