...
from nntool.graph.dim import Dim, PadDim
from nntool.graph.types import FilterLikeNodeBase, PadNode, ReshapeNode
...
class MatchFusePad(Matcher):
    ...
    def find_conv(G, node, padding, reshapes=None):
        ...
        if reshapes is None:
            reshapes = []
        if isinstance(node, FilterLikeNodeBase):
    ...
    def _match(self, G: GraphView, **kwargs) -> bool:
        ...
        for pad_params in [pad for pad in G.nodes() if isinstance(pad, PadNode)]:
