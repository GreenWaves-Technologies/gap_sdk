...
@generation_function("kernels", (TransposeParameters, ), qrec_types=(QREC_FLOAT, ))
def three_d_transpose_kernels_generator_fp16(gen, node, qrec, in_eparams, out_eparams, cname):
    del in_eparams, out_eparams
    real_in_shape, real_transpose = node.real_shape()
    if len(real_transpose) <= 1:
        return True
    if len(real_transpose) == 2:
        gen.kernels.append(TwoDTransposeKernelfp16(cname, node, real_in_shape,
                                                   real_transpose, qrec, at_ver=gen.opts['at_ver']))
    elif len(real_transpose) == 3:
        gen.kernels.append(ThreeDTransposeKernelfp16(cname, node, real_in_shape,
                                                     real_transpose, qrec, at_ver=gen.opts['at_ver']))
    else:
...
def gen_at_2d_transpose(code_block, name,
    ...
    code_block.write('CNN_MatTranspose_fp16("{}", {}, {}, {}, KOP_MATTRANSP);',
                     name, gen_ctrl, in_shape[1], in_shape[0])


class TwoDTransposeKernelfp16(AutotilerKernel):
...
def gen_at_3d_transpose(code_block, name,
    ...
    code_block.write('CNN_3DTensorPermute_fp16("{}", {}, {}, {}, {}, {});',
                     name, gen_ctrl, in_shape[0], in_shape[2], in_shape[1],
                     permop)


class ThreeDTransposeKernelfp16(AutotilerKernel):
    def __init__(self, cname, params, real_in_shape, real_transpose, qrec, gen_ctrl=None, at_ver=3):
        if gen_ctrl is None:
            self.gen_ctrl = GenCtrl(None, cname=cname)
        else:
