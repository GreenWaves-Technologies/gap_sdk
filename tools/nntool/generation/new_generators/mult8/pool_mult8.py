...
class PoolActGenerator(GeneratorBase, InOutBindingsMixin):
    ...
    def globals_generator(cls, gen, node, qrec, pnode, fnode) -> bool:
        if isinstance(pnode, (GlobalPoolingParameters, PoolingParameters, GlobalSumPoolParameters)):
            ...
            infos, comment = np.array([qrec.cache['scale_mul_biases_q'].qbiases[0],
                                       qrec.cache['scale_mul_biases_q'].qnorms[0],
                                       0, 0, 0]), "no activation"
            fnode = pnode
            pool_q = qrec
        elif isinstance(pnode, ActivationFusion) and isinstance(fnode, (GlobalPoolingParameters, PoolingParameters)):
    ...
    def bindings_generator(cls, gen, node, qrec, in_eparams, out_eparams, cname) -> bool:
        if isinstance(node, ActivationFusion):
            ...
            if isinstance(cnodes[0], (GlobalPoolingParameters, PoolingParameters)):
                cls.set_in_out_infos_bindings(gen, in_eparams, out_eparams, cname, cnodes[0], qrec)
                return True
            return False
