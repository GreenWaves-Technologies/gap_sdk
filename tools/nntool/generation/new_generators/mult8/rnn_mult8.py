...
import numpy as np
from execution.kernels.quant.rnn import internal_qtype
...
from generation.new_generators.generator_base import (GeneratorBase, ktype, qrec_options,
                                                      paramstype)
from graph.types import (GRUParameters, LSTMParameters, RNNBaseParameters,
                         RNNParameters)
from graph.types.lstm import LSTMParameters
from graph.types.rnn import GRUParameters, RNNParameters
from quantization.qtype import QType

LOG = logging.getLogger("nntool." + __name__)


@paramstype(RNNParameters, GRUParameters, LSTMParameters)
...
class RNNGenerator(GeneratorBase):
    ...
    @classmethod
    def num_sequences(cls, node: RNNBaseParameters):
    ...
    def set_lstm_bindings(cls, gen, step_idx, in_eparams, out_eparams, cname,
        ...
        num_seq = cls.num_sequences(rnn_params)
        if num_seq > 1:
    ...
    def kernel_generator(cls, gen, params, qrec, in_eparams, out_eparams, cname) -> bool:
        ...
        if isinstance(params, RNNParameters):
            kernel = RNNKernel
        elif isinstance(params, LSTMParameters):
            kernel = LSTMKernel
