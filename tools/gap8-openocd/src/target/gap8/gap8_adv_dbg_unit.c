...
/* Control regs */
// tap ID
#define TID_CTRL_REG 2
// AXI r/w through
#define AXI_CTRL_REG 4
// Soc control
#define SOC_CTRL_REG 7
...
static struct gap8_dbg_unit gap8_adv_dbg_unit;
static int gap8_adv_jtag_read_memory(struct gap8_jtag *jtag_info,
		uint32_t addr, uint32_t size, int count, uint8_t *buffer);

static int gap8_adv_jtag_write_memory(struct gap8_jtag *jtag_info,
		uint32_t addr, uint32_t size, int count, const uint8_t *buffer);


static int gap8_adv_cpu_enable_ebreak(struct gap8_jtag *jtag_info, int enable);

//static int adbg_ctrl_write(struct gap8_jtag *jtag_info, uint8_t regidx,
//		uint32_t *cmd_data, int length_bits);

//static int adbg_ctrl_read(struct gap8_jtag *jtag_info, uint32_t regidx,
//		uint32_t *data, int length_bits);

static int gap8_adv_cpu_stall(struct gap8_jtag *jtag_info, int action);

//static int adbg_select_ctrl_reg(struct gap8_jtag *jtag_info, uint8_t regidx);

static int adbg_select_module(struct gap8_jtag *jtag_info, int chain);
...
static int gap8_adv_jtag_init(struct gap8_jtag *jtag_info)
{
	printf("Init jtag\n");
	struct gap8_tap_ip *tap_ip = jtag_info->tap_ip;
	...
	jtag_info->gap8_jtag_inited = 1;

	/* TAP reset - not sure what state debug module chain is in now */
	jtag_info->gap8_jtag_module_selected = DC_NONE;
	...
	if (retval != ERROR_OK)
		return retval;
	if (gap8_adv_dbg_unit.options & BOOT_MODE_JTAG)
		LOG_INFO("adv debug unit is configured with option BOOT MODE JTAG");
	if (gap8_adv_dbg_unit.options & ADBG_USE_HISPEED)
		LOG_INFO("adv debug unit is configured with option ADBG_USE_HISPEED");

	// set jtag boot mode (remove high speed bit from options)
	uint32_t jtag_boot_mode = gap8_adv_dbg_unit.options >> 1; 
	// Must assert reset while writing boot mode to soc ctrl reg
	jtag_add_reset(0,1);
	...
	if (retval != ERROR_OK) {
		LOG_ERROR("JTAG power up request error");
		return retval;
	}
	gap8_adv_cpu_stall(jtag_info, CPU_STALL);

	gap8_adv_cpu_enable_ebreak(jtag_info, 1);
	return ERROR_OK;
}
...
static int adbg_select_module(struct gap8_jtag *jtag_info, int chain)
{
	if (jtag_info->gap8_jtag_module_selected == chain)
		return ERROR_OK;
	...
	jtag_info->gap8_jtag_module_selected = chain;
...
static int adbg_select_ctrl_reg(struct gap8_jtag *jtag_info, uint8_t regidx)
{
	...
	if (jtag_info->current_reg_idx[jtag_info->gap8_jtag_module_selected] == regidx)
		return ERROR_OK;

	switch (jtag_info->gap8_jtag_module_selected) {
		case DC_AXI4:
		...
		default:
			LOG_ERROR("Illegal debug chain selected (%i) while selecting control register",
					jtag_info->gap8_jtag_module_selected);
			return ERROR_FAIL;
	}
	...
	jtag_info->current_reg_idx[jtag_info->gap8_jtag_module_selected] = regidx;
 ...
 */
static int adbg_burst_command(struct gap8_jtag *jtag_info, uint32_t opcode,
...
static int adbg_wb_burst_read(struct gap8_jtag *jtag_info, int size,
...
static int adbg_wb_burst_write(struct gap8_jtag *jtag_info, const uint8_t *data,
		int size, int count, unsigned long start_address)
{
	int retry_full_crc = 0;
	int retval;
	uint8_t opcode;
...
retry_full_write:
	...
	if (jtag_info->gap8_jtag_module_selected == DC_AXI4 &&
			!(gap8_adv_dbg_unit.options & ADBG_USE_HISPEED)) {
		uint32_t addr;
...
static int gap8_adv_jtag_read_cpu(struct gap8_jtag *jtag_info,
		uint32_t addr, int count, uint32_t *value)
{
	int retval;
	if (!jtag_info->gap8_jtag_inited) {
		retval = gap8_adv_jtag_init(jtag_info);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int gap8_adv_jtag_write_cpu(struct gap8_jtag *jtag_info,
...
static int gap8_adv_cpu_enable_ebreak(struct gap8_jtag *jtag_info, int enable)
{
	...
	retval = gap8_adv_jtag_read_memory(jtag_info, GAP8_DEBUG_REG_ADDR(gap_core_id, DBG_IE), 4, 1, (uint8_t*)&reg);

	if (retval != ERROR_OK)
		return retval;

	if (enable)
		reg |= GAP8_DEBUG_IE_BP;
	else
		reg &= ~GAP8_DEBUG_IE_BP;
	...
	return gap8_adv_jtag_write_memory(jtag_info, GAP8_DEBUG_REG_ADDR(gap_core_id, DBG_IE), 4, 1, (uint8_t *)&reg);
}
...
static int gap8_adv_cpu_stall(struct gap8_jtag *jtag_info, int action)
{
	...
	retval = gap8_adv_jtag_read_memory(jtag_info, GAP8_DEBUG_REG_ADDR(gap_core_id, DBG_CTRL), 4, 1, (uint8_t*)&reg);

	if (retval != ERROR_OK)
		return retval;

	if (action == CPU_STALL)
		reg |= GAP8_DEBUG_CTRL_HALT;
	else
		reg &= ~GAP8_DEBUG_CTRL_HALT;
	...
	return gap8_adv_jtag_write_memory(jtag_info, GAP8_DEBUG_REG_ADDR(gap_core_id, DBG_CTRL), 4, 1, (uint8_t *)&reg);
}

static int gap8_adv_is_cpu_running(struct gap8_jtag *jtag_info, int *running)
{
	uint32_t reg;
	int retval;
	...
	retval = gap8_adv_jtag_read_memory(jtag_info, 0xf9010000, 4, 1, (uint8_t *)&reg);
	...
	if (retval != ERROR_OK)
	{
		printf("can't read halt state\n");
		return retval;
	}

	if (reg & GAP8_DEBUG_CTRL_HALT)
		*running = 0;
	else
		*running = 1;
...
static int gap8_adv_cpu_reset(struct gap8_jtag *jtag_info, int action)
{
	...
	{
#if 0
		retval = gap8_adv_jtag_write_memory(jtag_info,
				gap8_event_unit_reg_addr[gap_core_id] + 0x80,
				4,
				1,
				(const uint8_t*)&reset_requst);
#endif
		uint32_t jtag_boot_mode = gap8_adv_dbg_unit.options >> 1; 
		printf("RESET: jtag boot mode=%x\n",jtag_boot_mode);
		// Must assert reset while writing boot mode to soc ctrl reg
		jtag_add_reset(0,1);
		jtag_add_sleep(1000000);
		jtag_execute_queue();
		// write soc ctrl reg
		struct jtag_tap *tap = jtag_info->tap;
		struct scan_field field;
...
static struct gap8_dbg_unit gap8_adv_dbg_unit = {
	.name                     = "adv_dbg_unit",
	.options                  = NO_OPTION,
	.gap8_jtag_init           = gap8_adv_jtag_init,

	.gap8_is_cpu_running      = gap8_adv_is_cpu_running,
	.gap8_cpu_stall           = gap8_adv_cpu_stall,
	.gap8_cpu_reset           = gap8_adv_cpu_reset,

	.gap8_jtag_read_cpu       = gap8_adv_jtag_read_cpu,
	.gap8_jtag_write_cpu      = gap8_adv_jtag_write_cpu,

	.gap8_jtag_read_memory    = gap8_adv_jtag_read_memory,
	.gap8_jtag_write_memory   = gap8_adv_jtag_write_memory
};

int gap8_adv_dbg_unit_register(void)
{
	list_add_tail(&gap8_adv_dbg_unit.list, &gap8_dbg_unit_list);
	return 0;
}
