...
install_headers: $(HEADER_DIR)
	...
	if [ -e $(GAP_SDK_HOME)/gap8/rtos/pulp/archi ]; then \
		cd $(GAP_SDK_HOME)/gap8/rtos/pulp/archi && \
		export INSTALL_DIR=$(GAP_SDK_HOME)/gap8/rtos/pulp/archi_gap/workstation && \
		export TARGET_INSTALL_DIR=$(GAP_SDK_HOME)/gap8/rtos/pulp/archi_gap/target && \
		make build; \
	fi
	if [ -e $(GAP_SDK_HOME)/gap8/rtos/pulp/hal ]; then \
		cd $(GAP_SDK_HOME)/gap8/rtos/pulp/hal && \
		export TARGET_INSTALL_DIR=$(GAP_SDK_HOME)/gap8/rtos/pulp/hal_gap/target && \
		make build; \
	fi
	cd $(GAP_SDK_HOME)/gap8/rtos/pulp/archi_gap/target && rsync -avR * $(TARGET_INSTALL_DIR)
	cd $(GAP_SDK_HOME)/gap8/rtos/pulp/archi_gap/workstation && rsync -avR * $(INSTALL_DIR)
	cd $(GAP_SDK_HOME)/gap8/rtos/pulp/hal_gap/target && rsync -avR * $(TARGET_INSTALL_DIR)

install_rt_gap8:
	make -C  $(GAP_SDK_HOME)/gap8/rtos/pulp/pulp-os MK_ROOT=$(GAP_SDK_HOME)/gap8/rtos/pulp/pulp-os/mk/gap header build install

install_rt_vega:
	make -C  $(GAP_SDK_HOME)/gap8/rtos/pulp/pulp-os MK_ROOT=$(GAP_SDK_HOME)/gap8/rtos/pulp/pulp-os/mk/vega header build install
...
clean:
	rm -rf $(GAP_SDK_HOME)/gap8/rtos/pulp/archi_gap $(GAP_SDK_HOME)/gap8/rtos/pulp/hal_gap
