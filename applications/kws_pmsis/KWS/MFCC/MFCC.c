...
int MFCC_Logfp(unsigned int a)
{
    ...
    int ar2 = ((ar>>(QN>>1)) * (ar>>(QN>>1))) >> (1);
    int ar3 = ((ar2>>(QN>>1)) * (ar>>(QN>>1))) << (1);
    ar3 = ((ar3>>(QN>>1)) * (c1_3>>(QN>>1))) ;
...
#else
                DataV[iB] = gap_cplxmuls(Tmp, W) >> (v2s) {FFT2_SCALEDOWN, FFT2_SCALEDOWN};
#endif
...
void MFCC_ComputeMFCCInt(unsigned int *__restrict__ FramePower,  unsigned int *__restrict__ MFCC)
{
    ...
    */
    ...
    for (i=0; i<(unsigned int)FilterBanks; i++) {
        ...
        sprintf(Outr,"%f",Coeff/(float)(1<<(24))/(float) (1<<9));	

	printf("mfcc coeff %d %x %s (%d) norm %d\n",i,Coeff,Outr,k,MFCC_FilterBank[i].Norm);
#endif
...
void MFCC_ComputeDCT(v2s *in_dct, v2s *twiddct, v2s *twiddles, short int *SwapLUT) {
    ...
    for (i=0;i<NDCT;i++) {
        v2s out_dct_loc = gap_cplxmuls(in_dct[i],twiddct[i]);
