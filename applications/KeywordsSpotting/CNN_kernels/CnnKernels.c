...

{
	...
	rt_dma_memcpy((unsigned int) In+(0), (unsigned int) (L1_Memory + 0)+0, 7840, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((unsigned int) Filter+0, (unsigned int) (L1_Memory + 15680)+0, 320, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	/* ===================End Read First Tile=========================================== */
	...
	for (OutPlane=0; OutPlane<32; OutPlane++) {
		...
		if (!LastOutPlane) {
			rt_dma_memcpy((unsigned int) Filter + (1*320),
					(unsigned int) (L1_Memory + 15680) + 320*((OutPlane+1) % 2), 320, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
		}
		...
		Iter=0; {
...
#ifdef TILED
			rt_team_fork(gap8_ncore(), (void *) KerSetInBias_int, (void *) KerArg0);
#else
			rt_team_fork(gap8_ncore(), (void *) KerSetInBias, (void *) KerArg0);
#endif
			...
			InPlane=0; {
				...
				if (!LastInPlane) {
					rt_dma_memcpy((unsigned int) In + ((InPlane+1)*7840 + (0)*7280),
							(unsigned int) (L1_Memory + 0) + 7840*((N_TiIp+1) % 2), 7840, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				} else
				if (!Last) {
					rt_dma_memcpy((unsigned int) In + ((0+1)*7280),
							(unsigned int) (L1_Memory + 0) + 7840*((N_TiIp+1) % 2), 7840, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				}
				else if (!LastOutPlane) {
					rt_dma_memcpy((unsigned int) In + (0),
							(unsigned int) (L1_Memory + 0) + 7840*((N_TiIp+1) % 2), 7840, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				}
				/* ===================End Read Next Tile=========================================== */
				...
				rt_team_fork(gap8_ncore(), (void *) KerDirectConvNxM_fp_tiled, (void *) KerArg1);
#else
				rt_team_fork(gap8_ncore(), (void *) KerDirectConvNxM_fp, (void *) KerArg1);
#endif
			...
			rt_team_fork(gap8_ncore(), (void *) KerReLUMaxPool2x2_fp, (void *) KerArg2);
			/* =======================Write Tile=========================================== */
			if (0||OutPlane) {
				rt_dma_wait(&DmaW_Evt1);
			}
			rt_dma_memcpy((unsigned int) Out + ((0)*1248),
     //				(unsigned int) (L1_Memory + 21536) + 1248*(N_Ti % 2), 1248, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
				(unsigned int) (L1_Memory + 26320) + 1248*(N_Ti % 2), 1248, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
			/* ===================End Write Tile=========================================== */
			N_Ti++;
...

{
	...
	/* =======================Read First Tile=========================================== */
	/* Initial reads in L2, O_DB or O_BUFF */
	rt_dma_memcpy((unsigned int) In+(0), (unsigned int) (L1_Memory + 0)+0, 1248, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((unsigned int) Filter+0, (unsigned int) (L1_Memory + 2496)+0, 1280, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	/* ===================End Read First Tile=========================================== */
	/* Kernel Iteration Loop on Output Planes */
	for (OutPlane=0; OutPlane<32; OutPlane++) {
		int LastOutPlane = ((OutPlane+1) == 32), NextLastOutPlane = ((OutPlane+2) == 32);
		...
		rt_dma_wait(&DmaR_Evt2);
		if (!LastOutPlane) {
			rt_dma_memcpy((unsigned int) Filter + (1*1280),
					(unsigned int) (L1_Memory + 2496) + 1280*((OutPlane+1) % 2), 1280, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
		}
		/* =======================End Read Next Partial Buff======================================= */
		/* Kernel Iteration Loop on tiled inner space 0 */
		Iter=0; {
			...
			for (InPlane=0; InPlane<32; InPlane++) {
				...
				rt_dma_wait(&DmaR_Evt1);
				if (!LastInPlane) {
					rt_dma_memcpy((unsigned int) In + ((InPlane+1)*1248 + (0)*1152),
							(unsigned int) (L1_Memory + 0) + 1248*((N_TiIp+1) % 2), 1248, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				} else
				if (!Last) {
					rt_dma_memcpy((unsigned int) In + ((0+1)*1152),
							(unsigned int) (L1_Memory + 0) + 1248*((N_TiIp+1) % 2), 1248, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				}
				else if (!LastOutPlane) {
					rt_dma_memcpy((unsigned int) In + (0),
...
#ifdef TILED
				rt_team_fork(gap8_ncore(), (void *) KerDirectConvNxM_tiled_2, (void *) KerArg1);
#else
				rt_team_fork(gap8_ncore(), (void *) KerDirectConvNxM_char, (void *) KerArg1);
#endif
			...
			rt_team_fork(gap8_ncore(), (void *) KerReLU_fp, (void *) KerArg2);
			/* =======================Write Tile=========================================== */
			if (0||OutPlane) {
				rt_dma_wait(&DmaW_Evt1);
			}
			rt_dma_memcpy((unsigned int) Out + ((0)*780),
				(unsigned int) (L1_Memory + 6836) + 780*(N_Ti % 2), 780, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
			/* ===================End Write Tile=========================================== */
			N_Ti++;
			/* End Kernel Iteration Loop on tiled inner space 0 */
		}
...

{
	...
	/* Initial reads in L2, O_DB or O_BUFF */
	rt_dma_memcpy((unsigned int) In+(0), (unsigned int) (L1_Memory + 0)+0, 1248, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((unsigned int) Filter+0, (unsigned int) (L1_Memory + 2496)+0, 2560, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	/* ===================End Read First Tile=========================================== */
	/* Kernel Iteration Loop on Output Planes */
	for (OutPlane=0; OutPlane<32; OutPlane++) {
		int LastOutPlane = ((OutPlane+1) == 32), NextLastOutPlane = ((OutPlane+2) == 32);
		...
		rt_dma_wait(&DmaR_Evt2);
		if (!LastOutPlane) {
			rt_dma_memcpy((unsigned int) Filter + (1*2560),
					(unsigned int) (L1_Memory + 2496) + 2560*((OutPlane+1) % 2), 2560, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
		}
		/* =======================End Read Next Partial Buff======================================= */
		/* Kernel Iteration Loop on tiled inner space 0 */
		Iter=0; {
			...
			rt_dma_memcpy((unsigned int) Out + ((0)*780),
				(unsigned int) (L1_Memory + 8396) + 780*(N_Ti % 2), 780, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
			/* ===================End Write Tile=========================================== */
			N_Ti++;
			/* End Kernel Iteration Loop on tiled inner space 0 */
		}
...

{
	...
	/* =======================Read First Tile=========================================== */
	/* Initial reads in L2, O_DB or O_BUFF */
	rt_dma_memcpy((unsigned int) In+0, (unsigned int) (L1_Memory + 0)+0, 24960, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((unsigned int) Bias+(0), (unsigned int) (L1_Memory + 24960)+0, 24, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	rt_dma_memcpy((unsigned int) Filter+(0), (unsigned int) (L1_Memory + 24984)+0, 12480, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt3);
	...
	for (Iter=0; Iter<12; Iter++) {
		...
		if (!Last) {
			rt_dma_memcpy((unsigned int) Filter + ((Iter+1)*12480),
					(unsigned int) (L1_Memory + 24984) + 12480*((N_Ti+1) % 2), 12480, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt3);
		}
		/* ===================End Read Next Tile=========================================== */
		/* Call Kernel LOC_INNER_LOOP */
		...
		rt_team_fork(gap8_ncore(), (void *) KerLinearLayer_fps, (void *) KerArg0);
		N_Ti++;
		/* End Kernel Iteration Loop on tiled inner space 0 */
	}
	Iter=12;
	...
	rt_dma_memcpy((unsigned int) Out + (0),
		(unsigned int) (L1_Memory + 49944) + 0, 24, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
	rt_dma_wait(&DmaW_Evt1);
	/* ===================End Write Last Tile=========================================== */
}
...

{
	...
	/* =======================Read First Tile=========================================== */
	/* Initial reads in L2, O_DB or O_BUFF */
	rt_dma_memcpy((unsigned int) In+0, (unsigned int) (L1_Memory + 0)+0, 12480, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((unsigned int) Bias+(0), (unsigned int) (L1_Memory + 12480)+0, 24, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	rt_dma_memcpy((unsigned int) Filter+(0), (unsigned int) (L1_Memory + 12504)+0, 12480, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt3);
	/* Wait for BUFF read in L2 */
	rt_dma_wait(&DmaR_Evt1);
	rt_dma_wait(&DmaR_Evt2);
	/* ===================End Read First Tile=========================================== */
	...
	for (Iter=0; Iter<12; Iter++) {
		...
		if (!Last) {
			rt_dma_memcpy((unsigned int) Filter + ((Iter+1)*12480),
					(unsigned int) (L1_Memory + 12504) + 12480*((N_Ti+1) % 2), 12480, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt3);
		}
		/* ===================End Read Next Tile=========================================== */
		/* Call Kernel LOC_INNER_LOOP */
		KerArg0->In = (Word16 * __restrict__) ((unsigned int) (L1_Memory + 0) + 0);
	...
	/* =======================Write Last Tile=========================================== */
	rt_dma_memcpy((unsigned int) Out + (0),
		(unsigned int) (L1_Memory + 37464) + 0, 24, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
	rt_dma_wait(&DmaW_Evt1);
	/* ===================End Write Last Tile=========================================== */
}
