...

{
	...
	rt_dma_memcpy_2d((rt_pointerT) In+(0), (rt_pointerT) (L1_Memory + 0)+0, 7840, 
		80, 80, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((rt_pointerT) Filter+0, (rt_pointerT) (L1_Memory + 15680)+0, 416, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	/* ===================End Read First Tile=========================================== */
	...
	for (OutPlane=0; OutPlane<32; OutPlane++) {
		...
		if (!LastOutPlane) {
			rt_dma_memcpy((rt_pointerT) Filter + (1*416),
					(rt_pointerT) (L1_Memory + 15680) + (416*((OutPlane+1) % 2)), 416, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
		}
		...
		Iter=0; {
			...
			InPlane=0; {
				...
				if (!LastInPlane) {
					rt_dma_memcpy_2d((rt_pointerT) In + ((InPlane+1)*7840 + (0)*72),
							(rt_pointerT) (L1_Memory + 0) + (7840*((N_TiIp+1) % 2)), 7840, 
							80, 80, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				} else
				if (!Last) {
					rt_dma_memcpy_2d((rt_pointerT) In + ((0+1)*72),
							(rt_pointerT) (L1_Memory + 0) + (7840*((N_TiIp+1) % 2)), 7840, 
							80, 80, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				}
				else if (!LastOutPlane) {
					rt_dma_memcpy_2d((rt_pointerT) In + (0),
							(rt_pointerT) (L1_Memory + 0) + (7840*((N_TiIp+1) % 2)), 7840, 
							80, 80, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				}
				/* ===================End Read Next Tile=========================================== */
			...
			rt_team_fork(gap8_ncore(), (void *) KerReLUMaxPool2x2_fp, (void *) KerArg5);
			/* =======================Write Tile=========================================== */
			if (0||OutPlane) {
				rt_dma_wait(&DmaW_Evt1);
			}
			rt_dma_memcpy_2d((rt_pointerT) Out + ((0)*32),
				(rt_pointerT) (L1_Memory + 21568) + (1248*(N_Ti % 2)), 1248, 
				32, 32, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
			/* ===================End Write Tile=========================================== */
			N_Ti++;
...

{
	...
	/* =======================Read First Tile=========================================== */
	/* Initial reads in L2, O_DB or O_BUFF */
	rt_dma_memcpy_2d((rt_pointerT) In+(0), (rt_pointerT) (L1_Memory + 0)+0, 1248, 
		32, 32, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
	rt_dma_memcpy((rt_pointerT) Filter+0, (rt_pointerT) (L1_Memory + 2496)+0, 3328, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
	/* ===================End Read First Tile=========================================== */
	/* Kernel Iteration Loop on Output Planes */
	for (OutPlane=0; OutPlane<32; OutPlane++) {
		int LastOutPlane = ((OutPlane+1) == 32), NextLastOutPlane = ((OutPlane+2) == 32);
		...
		rt_dma_wait(&DmaR_Evt2);
		if (!LastOutPlane) {
			rt_dma_memcpy((rt_pointerT) Filter + (1*3328),
					(rt_pointerT) (L1_Memory + 2496) + (3328*((OutPlane+1) % 2)), 3328, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt2);
		}
		/* =======================End Read Next Partial Buff======================================= */
		/* Kernel Iteration Loop on Iter space */
		Iter=0; {
			...
			for (InPlane=0; InPlane<32; InPlane++) {
				...
				rt_dma_wait(&DmaR_Evt1);
				if (!LastInPlane) {
					rt_dma_memcpy_2d((rt_pointerT) In + ((InPlane+1)*1248 + (0)*24),
							(rt_pointerT) (L1_Memory + 0) + (1248*((N_TiIp+1) % 2)), 1248, 
							32, 32, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				} else
				if (!Last) {
					rt_dma_memcpy_2d((rt_pointerT) In + ((0+1)*24),
							(rt_pointerT) (L1_Memory + 0) + (1248*((N_TiIp+1) % 2)), 1248, 
							32, 32, RT_DMA_DIR_EXT2LOC, 0, &DmaR_Evt1);
				}
				else if (!LastOutPlane) {
			...
			rt_team_fork(gap8_ncore(), (void *) KerReLU_fp, (void *) KerArg5);
			/* =======================Write Tile=========================================== */
			if (0||OutPlane) {
				rt_dma_wait(&DmaW_Evt1);
			}
			rt_dma_memcpy_2d((rt_pointerT) Out + ((0)*28),
				(rt_pointerT) (L1_Memory + 9992) + (840*(N_Ti % 2)), 840, 
				28, 28, RT_DMA_DIR_LOC2EXT, 0, &DmaW_Evt1);
			/* ===================End Write Tile=========================================== */
			N_Ti++;
			/* End Kernel Iteration Loop on Iter space */
		}
