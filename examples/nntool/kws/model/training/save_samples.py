...
def main():
    ...
    for i in range(batch_size):
        ...
        with open(os.path.join(directory, "features_q8_{}_{}_{}.pgm".format(expected[i], predicted[i], i)), 'wb') as f:
            hdr =  'P5' + '\n' + str(40) + '  ' + str(98) + '  ' + str(255) + '\n'
            f.write(hdr.encode())
            np.int8(s_8b).tofile(f)
        strout += 'Input:\t./images/features_float_{}.dat\tExpected:\t{}\tPredicted:\t{}\t({})\n'.format(str(i), expected[i], predicted[i], outfc[i])
    with open(os.path.join(directory, "output_expected_predicted.txt"), 'w') as f:
        f.write(strout)
