...
void test_therm_eye()
{
    ...
    #if defined(ASYNC)
    pi_task_block(&cb);
    pi_camera_control(&cam, PI_CAMERA_CMD_CONTINUE_MODE, 0);
    ...
    //This mode stops just the CPI interface leaving the sensor on
    pi_camera_control(&cam, PI_CAMERA_CMD_CONTINUE_MODE, 0);
    #endif  /* ASYNC */
    ...
    while (loop)
    {
        ...
        //Turn on cpi and trigger a snapshot
        pi_camera_control(&cam, PI_CAMERA_CMD_TRIGGER_MODE, 0);
        pi_task_wait_on(&cb);
        //Turn of cpi leaving sensor on
        pi_camera_control(&cam, PI_CAMERA_CMD_CONTINUE_MODE, 0);
        #else
