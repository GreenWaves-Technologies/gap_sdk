...
static void end_of_tx(void *arg)
{
  ...
  if (i){
      rt_hyperram_read_2d(hyper, read_buff[i], hyper_buff[i], 987, 126, 24, rt_event_get(&sched, end_of_rx, (void *)i));
      /*
      unsigned int start = gap8_readhwtimer();
      rt_hyperram_read_2d(hyper, read_buff[i], hyper_buff[i], 60*1024, 126*6, 153, NULL);
      unsigned int end = gap8_readhwtimer() - start;
      printf("it cost %d cycles\n", end);
      */
  }
  else
