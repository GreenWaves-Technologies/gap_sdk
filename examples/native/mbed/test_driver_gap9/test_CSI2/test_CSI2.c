...
#if (ULPS == 1)
/* Do not touch, for ultra-low-power test, only use RAW8*/
#define RAW8    1
#else
...
#define UR_X             480
#define UR_Y             7
#define SLICE_SIZE       ((UR_X-LL_X + PIXEL_PER_CYCLE) * (UR_Y-LL_Y + 1))
...
int csi2_pixel_ulps(void)
{
    CSI2->RX_ENABLE = CSI2_RX_ENABLE_CH0(1);

    csi2_read((uint32_t)rx_buffer, FRAME_SIZE, 0);
    while(CSI2_PIXEL->ULPS_STATUS != 0x13);
    /* Enter ultra-low-power */
    wait_us(1000);
    while(CSI2_PIXEL->ULPS_STATUS != 0x00);
...
int csi2_pixel_read(void)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if(i > 5) {
            ...
            CSI2->SLICE0_LXY = ( (LL_X / PIXEL_PER_CYCLE) << 16) | LL_Y;
            CSI2->SLICE0_RXY = ( (UR_X / PIXEL_PER_CYCLE) << 16) | UR_Y;
            CSI2->SLICE_ENABLE = CSI2_SLICE_ENABLE_CH0(1);

            csi2_read((uint32_t)rx_buffer, SLICE_SIZE, 0);
        } else {
            csi2_read((uint32_t)rx_buffer, FRAME_SIZE, 0);
        }
...
int main()
{
    ...
    int error = csi2_pixel_ulps();
    #else
    /* Read pixel */
    int error = csi2_pixel_read();
    #endif
