...
int main( int argc, char ** argv )
{
    ...
    string filename = "../rand-graph-" + to_string(numNodes) + ".h";
    ...
    outf << "typedef struct {\n";
    outf << "int starting;\n";
    outf << "int no_of_edges;\n";
    outf << "}Node;\n";
    
    outf << "const int no_of_nodes = "<< numNodes << ";\n";
    ulong totalEdges = 0;
    outf << "const Node h_graph_nodes[ "<< numNodes << "] = {\n";
    ...
    outf << "};\n";
    outf << "const int source =" << randNode( gen ) << ";\n";
    outf << "const int edge_list_size =" << totalEdges << ";\n";
    outf << "const int h_graph_edges[ "<< totalEdges << "] = {\n";
    for ( ulong i = 0; i < numNodes; i++ )
        for ( uint j = 0; j < graph[i].size(); j++ )
        {
            outf << graph[i][j].dest;
            if(i == (numNodes-1) && j == (graph[i].size()-1))
                outf << "\n";
            else
                outf << ",\n";
        }
        outf << "};\n";
    outf << "const int edgeWeight[ "<< totalEdges << "] = {\n";
    for ( ulong i = 0; i < numNodes; i++ )
        for ( uint j = 0; j < graph[i].size(); j++ )
        {
            outf << graph[i][j].weight;
            if(i == (numNodes-1) && j == (graph[i].size()-1))
                outf << "\n";
            else
                outf << ",\n";
