...
struct pi_default_flash_conf conf_flash;
...
void CopyFileFromFlashToL3(struct pi_device *fs, char *file_name, uint32_t *_buff, uint32_t *_size)
{
    ...
    if (pi_ram_alloc(&DefaultRam, _buff, file->size))
    {
    ...
    *_size = file->size;
    ...
    do
    {
        ...
        if (size)
        {
            pi_ram_write(&DefaultRam, (uint32_t) (*_buff+size_total), buff, size);
            size_total += size;
        }
...
void test_cifar10(void)
{
    ...
    pi_open_from_conf(&DefaultRam, &conf_ram);
    ...
    pi_default_flash_conf_init(&conf_flash);
    ...
    if (pi_ram_alloc(&DefaultRam, (uint32_t *) &Out_Layer[0], Out_Layer_Size[0]))
    {
        printf("Ram malloc failed !\n");
        pmsis_exit(-5);
    }
    ...
    {
        pi_ram_free(&DefaultRam, Filter_Layer[i], Filter_Layer_Size[i]);
        pi_ram_free(&DefaultRam, Bias_Layer[i], Bias_Layer_Size[i]);
    }
    pi_ram_free(&DefaultRam, (uint32_t) Out_Layer[0], Out_Layer_Size[0]);
    pi_ram_close(&DefaultRam);
